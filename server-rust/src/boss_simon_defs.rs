...
// Execute Boss Simon toxic spray behavior
...
pub fn cleanup_simon_ai_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    cleanup_chemical_zombie_spawning(ctx, boss_monster_id);
    cleanup_toxic_zones(ctx, boss_monster_id);
    cleanup_chemical_bolts(ctx, boss_monster_id);
    cleanup_toxic_sprays(ctx, boss_monster_id);
    
    log::info!("Cleaned up all Simon boss AI schedules for boss {}", boss_monster_id);
}