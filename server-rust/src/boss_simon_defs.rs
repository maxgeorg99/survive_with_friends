...
use crate::{
    bestiary, config, monsters, monsters_boid, player, DbVector2, MonsterAttackType, MonsterType,
    Player,
};
use spacetimedb::{
...
#[table(accessor = boss_simon_last_patterns, public)]
...
#[table(accessor = simon_zombie_wave_scheduler, scheduled(spawn_chemical_zombie_wave), public)]
...
#[table(accessor = toxic_zone_scheduler, scheduled(trigger_toxic_zone), public)]
...
#[table(accessor = chemical_bolt_scheduler, scheduled(trigger_chemical_bolt), public)]
...
#[table(accessor = toxic_spray_scheduler, scheduled(trigger_toxic_spray), public)]
    ...
    ctx.db
        ...
        .insert(crate::ActiveMonsterAttack {
            ...
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(TOXIC_ZONE_DURATION_MS),
            ),
...
pub fn trigger_chemical_bolt(ctx: &ReducerContext, scheduler: ChemicalBoltScheduler) {
    ...
    ctx.db
        ...
        .insert(crate::ActiveMonsterAttack {
            active_monster_attack_id: 0,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(CHEMICAL_BOLT_DURATION_MS),
            ),
            position: boss_pos,
            direction: dir,
...
pub fn trigger_toxic_spray(ctx: &ReducerContext, scheduler: ToxicSprayScheduler) {
    ...
    let angle_step =
        TOXIC_SPRAY_ARC_DEGREES * std::f32::consts::PI / (180.0 * TOXIC_SPRAY_PROJECTILES as f32);

    // Create spray projectiles in an arc
    for i in 0..TOXIC_SPRAY_PROJECTILES {
        let angle = base_angle + (i as f32 * angle_step)
            - (TOXIC_SPRAY_ARC_DEGREES * std::f32::consts::PI / 360.0);
        ...
        ctx.db
            ...
            .insert(crate::ActiveMonsterAttack {
                active_monster_attack_id: 0,
                scheduled_at: ScheduleAt::Time(
                    ctx.timestamp + Duration::from_millis(TOXIC_SPRAY_DURATION_MS),
                ),
                position: boss_pos,
                direction: dir,
    ...
    if scheduler.spray_count > 0 {
        ctx.db.toxic_spray_scheduler().insert(ToxicSprayScheduler {
            ...
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(TOXIC_SPRAY_INTERVAL_MS),
            ),
...
pub fn schedule_next_zombie_wave(ctx: &ReducerContext, boss_monster_id: u32) {
    ctx.db
        ...
        .insert(SimonZombieWaveScheduler {
            scheduled_id: 0,
            boss_monster_id,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(ZOMBIE_SPAWN_INTERVAL_MS),
            ),
        });
}
...
pub fn schedule_next_toxic_zone(ctx: &ReducerContext, boss_monster_id: u32) {
    ctx.db.toxic_zone_scheduler().insert(ToxicZoneScheduler {
        ...
        boss_monster_id,
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(TOXIC_ZONE_INTERVAL_MS),
        ),
    });
}

pub fn schedule_next_chemical_bolt(
    ctx: &ReducerContext,
    boss_monster_id: u32,
    ...
    ctx.db
        ...
        .insert(ChemicalBoltScheduler {
            scheduled_id: 0,
            boss_monster_id,
            target_player_id,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(CHEMICAL_BOLT_INTERVAL_MS),
            ),
        });
}
...
pub fn cleanup_toxic_zones(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    let active_zones: Vec<u64> = ctx
        .db
        .active_monster_attacks()
        .iter()
        .filter(|a| {
            a.monster_attack_type == MonsterAttackType::SimonToxicZone
...
pub fn execute_boss_simon_chemical_bolt_pattern(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Boss Simon {} starting chemical bolt pattern",
        monster.monster_id
    );
...
pub fn execute_boss_simon_toxic_zone_pattern(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Boss Simon {} starting toxic zone pattern",
        monster.monster_id
    );
...
pub fn execute_boss_simon_toxic_spray_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Boss Simon {} starting toxic spray pattern",
        monster.monster_id
    );
...
pub fn execute_boss_simon_phase2_transform(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Boss Simon {} entering Phase 2 transform",
        monster.monster_id
    );
    ...
    let bestiary_entry = ctx
        ...
        .bestiary()
        .bestiary_id()
        .find(&(MonsterType::BossSimonPhase2 as u32))
...
pub fn initialize_simon_boss_ai(ctx: &ReducerContext, monster_id: u32) {
    ...
    if let Some(mut monster) = monster_opt {
        ...
        log::info!(
            "Initialized Simon boss {} in Phase 1 with chemical bolt pattern",
            monster_id
        );
    }
}
...
pub fn initialize_phase2_boss_simon_ai(ctx: &ReducerContext, monster_id: u32) {
    log::info!(
        "Initializing Phase 2 Boss Simon AI for monster {}",
        monster_id
    );
    ...
    if monster_opt.is_none() {
        log::warn!(
            "initialize_phase2_boss_simon_ai: Monster {} not found",
            monster_id
        );
        return;
    }
...
pub fn cleanup_simon_ai_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    log::info!(
        "Cleaning up all Simon boss AI schedules for boss {}",
        boss_monster_id
    );

    // Phase 1 & 2 Chemical Bolt cleanup
    let bolt_schedulers: Vec<u64> = ctx
        .db
        .chemical_bolt_scheduler()
        .boss_monster_id()
        .filter(&boss_monster_id)
    ...
    let active_bolts: Vec<u64> = ctx
        ...
        .iter()
        .filter(|a| {
            a.monster_attack_type == MonsterAttackType::SimonChemicalBolt
                && a.parameter_u == boss_monster_id
        })
        .map(|a| a.active_monster_attack_id)
        .collect();
    ...
    let zone_schedulers: Vec<u64> = ctx
        .db
        .toxic_zone_scheduler()
        .boss_monster_id()
        .filter(&boss_monster_id)
    ...
    let spray_schedulers: Vec<u64> = ctx
        .db
        .toxic_spray_scheduler()
        .boss_monster_id()
        .filter(&boss_monster_id)
    ...
    let active_sprays: Vec<u64> = ctx
        ...
        .iter()
        .filter(|a| {
            a.monster_attack_type == MonsterAttackType::SimonToxicSpray
                && a.parameter_u == boss_monster_id
        })
        .map(|a| a.active_monster_attack_id)
        .collect();
    ...
    let zombie_schedulers: Vec<u64> = ctx
        .db
        .simon_zombie_wave_scheduler()
        .boss_monster_id()
        .filter(&boss_monster_id)
...
pub fn cleanup_simon_phase2_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    log::info!(
        "Cleaning up Simon Phase 2 schedules for boss {}",
        boss_monster_id
    );
    ...
    log::info!(
        "Cleaned up Simon Phase 2 schedules: {} toxic sprays, {} active sprays, {} zombie waves",
        spray_schedulers.len(),
        active_sprays.len(),
        zombie_schedulers.len()
    );
}
...
fn schedule_random_simon_pattern(
    ctx: &ReducerContext,
    monster_id: u32,
) -> crate::monster_ai_defs::AIState {
    let mut rng = ctx.rng();
    ...
    if let Some(mut last_pattern) = ctx
        .db
        .boss_simon_last_patterns()
        .monster_id()
        .find(&monster_id)
