use crate::{
    boss_system::game_state, config_def::config, monsters_def::monsters,
    monsters_def::monsters_boid, DbVector2, MonsterType, WORLD_CELL_SIZE, WORLD_GRID_HEIGHT,
    WORLD_GRID_WIDTH,
};
...
pub fn spawn_world_structures(ctx: &ReducerContext) {
    ...
    for cell_y in 0..WORLD_GRID_HEIGHT {
        for cell_x in 0..WORLD_GRID_WIDTH {
            ...
            if center_x < margin
                || center_x > (config.world_size as f32 - margin)
                || center_y < margin
                || center_y > (config.world_size as f32 - margin)
            {
                continue;
            }
            ...
            if let Some(monster_type) = structure_type {
                ...
                let spawn_position = DbVector2::new(center_x + offset_x, center_y + offset_y);
                ...
                let clamped_position = DbVector2::new(
                    spawn_position.x.clamp(
                        structure_radius,
                        config.world_size as f32 - structure_radius,
                    ),
    ...
    log::info!(
        "World structures spawned: {} total ({} Crates, {} Trees, {} Statues)",
        structures_spawned,
        crates_spawned,
        trees_spawned,
        statues_spawned
...
fn spawn_structure(ctx: &ReducerContext, position: DbVector2, structure_type: MonsterType) {
    ...
    let (health, radius) = match structure_type {
        ...
        _ => {
            log::error!(
                "spawn_structure: Invalid structure type {:?}",
                structure_type
            );
            return;
        }
...
pub fn trigger_structure_death_loot(ctx: &ReducerContext, monster: &crate::Monsters) {
    ...
    let structure_position = if let Some(boid) = ctx
        .db
        .monsters_boid()
        .monster_id()
        .find(&monster.monster_id)
    {
        ...
        log::warn!(
            "Structure {} has no boid data, using spawn position for death loot",
            monster.monster_id
        );
    ...
    let loot_count = match monster.bestiary_id {
        ...
        _ => {
            log::error!(
                "trigger_structure_death_loot: Invalid structure type {:?}",
                monster.bestiary_id
            );
            return;
        }
    ...
    if crate::curses_defs::is_curse_active(ctx, crate::curses_defs::CurseType::NoStructureLoot) {
        ...
        log::info!(
            "{:?} destroyed! Spawning {} loot capsules at position ({:.1}, {:.1})",
            monster.bestiary_id,
            loot_count,
    ...
    if monster.bestiary_id == MonsterType::Statue {
        ...
        log::info!(
            "Statue destroyed! Spawned Lore Scroll (ID: {}, Type: {}) at position ({:.1}, {:.1})",
            scroll_id,
            scroll_type,
            scroll_position.x,
            scroll_position.y
    ...
    log::info!(
        "Structure death loot complete - {} capsules spawned{}",
        loot_count,
        if monster.bestiary_id == MonsterType::Statue {
            " + 1 lore scroll"
        } else {
