use crate::{account, class_data, player, PlayerClass};
use spacetimedb::{rand::Rng, reducer, ReducerContext, Table};
...
pub fn spawn_bot(ctx: &ReducerContext) {
    ...
    if current_player_count >= crate::MAX_PLAYERS as u64 {
        panic!(
            "SpawnBot: Server has reached maximum player capacity ({}/{}). Cannot spawn new bot.",
    ...
    */
    ...
    log::info!(
        "Creating bot player '{}' at position: {}, {}",
        bot_name,
        spawn_position.x,
        spawn_position.y
    );
    ...
    let new_player_opt = crate::create_new_player_with_position(
        ctx,
        &bot_name,
        player_class.clone(),
        spawn_position,
    ...
    log::info!(
        "Created new bot player record with class {:?}",
        player_class.clone()
    );
}
...
pub fn debug_enable_bot_pvp(ctx: &ReducerContext) {
    ...
    log::info!(
        "Debug: PvP enabled for {}/{} bots (rest already had PvP enabled)",
        updated_count,
        bot_count
    );
}
