...
#[table(accessor = dead_player_transition_timer, scheduled(transition_dead_to_choosing_class))]
...
#[table(accessor = winner_transition_timer, scheduled(transition_winner_to_curse_cutscene))]
...
#[table(accessor = curse_transition_timer, scheduled(transition_curse_to_choosing_class))]
...
pub fn client_connected(ctx: &ReducerContext) {
    ...
    if let Some(account) = ctx.db.account().identity().find(&identity) {
        ...
        log::info!(
            "Account details: Name={}, PlayerID={}, State={:?}",
            account.name,
...
pub fn set_name(ctx: &ReducerContext, name: String) {
    let identity = ctx.sender();
    log::info!(
        "SetName called by identity: {} with name: {}",
        identity,
        name
    );
    ...
    if name.trim().is_empty() || name.len() > 16 {
        panic!(
            "SetName: Invalid name provided by {}: '{}'. Name must be 1-16 characters.",
            identity, name
        );
    }

    // Find the account using the context's Db object and the primary key index (identity)
    let mut account = ctx.db.account().identity().find(&identity).expect(&format!(
        "SetName: Attempted to set name for non-existent account {}.",
        identity
    ));
    ...
    if account.state != AccountState::ChoosingName {
        panic!(
            "SetName: Account {} is not in ChoosingName state. Current state: {:?}",
    ...
    log::info!(
        "Account {} name set to {} and transitioned to ChoosingClass state.",
        identity,
        account_name
    );
}
...
pub fn update_last_login(ctx: &ReducerContext) {
    ...
    let mut account = ctx.db.account().identity().find(&identity).expect(&format!(
        "UpdateLastLogin: Account not found for identity {}",
        identity
    ));
...
pub fn spawn_player(ctx: &ReducerContext, class_id: u32) {
    ...
    let account = ctx.db.account().identity().find(&identity).expect(&format!(
        "SpawnPlayer: Account {} does not exist.",
        identity
    ));
    ...
    if account.state != AccountState::ChoosingClass {
        panic!(
            "SpawnPlayer: Account {} is not in ChoosingClass state. Current state: {:?}",
            identity, account.state
        );
    }
    ...
    if account.current_player_id != 0 {
        ...
        if ctx
            ...
            .player()
            .player_id()
            .find(&account.current_player_id)
            .is_some()
        {
            panic!(
                "SpawnPlayer: Account {} already has an active player (ID: {})",
                identity, account.current_player_id
            );
        } else {
    ...
    log::info!(
        "Created new player record for {} with class {:?} and transitioned to Playing state.",
        identity,
        player_class
    );
    ...
    if ctx.db.player().count() == 1 {
        ...
        let timers_to_delete: Vec<u64> = ctx
            .db
            .boss_spawn_timer()
            .iter()
        ...
        let monster_timers_to_delete: Vec<u64> = ctx
            .db
            .monster_spawn_timer()
            .iter()
            .map(|timer| timer.scheduled_id)
...
fn create_new_player(
    ...
    let spawn_position = match spawn_position {
        ...
        None => {
            log::error!(
                "CreateNewPlayer: Failed to find safe spawn position for player '{}'",
                name
            );
    ...
    log::info!(
        "Placing new player '{}' at safe position: {}, {}",
        name,
        spawn_position.x,
        spawn_position.y
    );
...
pub fn create_new_player_with_position(
    ctx: &ReducerContext,
    name: &str,
    player_class: PlayerClass,
    position: DbVector2,
    ...
    let class_data = ctx
        ...
        .class_data()
        .class_id()
        .find(&(player_class.clone() as u32));

    // Define default stats in case class data isn't found
    let (base_max_hp, armor, base_speed, starting_attack_type) =
        if let Some(class_data) = class_data {
            (
                class_data.max_hp as f32,
                class_data.armor,
                class_data.speed,
                class_data.starting_attack_type,
            )
        } else {
            log::error!(
                "CreateNewPlayerWithPosition: No class data found for {:?}",
                player_class.clone()
            );
    ...
    let max_hp = if crate::curses_defs::is_curse_active(
        ctx,
        crate::curses_defs::CurseType::PlayersStartLessHp,
    ) {
    ...
    let speed = if crate::curses_defs::is_curse_active(
        ctx,
        crate::curses_defs::CurseType::PlayersStartLessSpeed,
    ) {
    ...
    let shield_count = if starting_attack_type == AttackType::Shield {
        2
    } else {
        0
    };
    ...
    let new_player = ctx.db.player().insert(Player {
        ...
        armor: armor as u32,
        unspent_upgrades: 0,
        rerolls: if crate::curses_defs::is_curse_active(
            ctx,
            crate::curses_defs::CurseType::NoFreeReroll,
        ) {
    ...
    attacks_def::schedule_new_player_attack(
        ctx,
        new_player.player_id,
        starting_attack_type.clone(),
        1,
    );
    log::info!(
        "Scheduled starting attack type {:?} for player {}",
        starting_attack_type.clone(),
        new_player.player_id
    );
...
pub fn transition_dead_to_choosing_class(ctx: &ReducerContext, timer: DeadPlayerTransitionTimer) {
    ...
    log::info!(
        "Transitioning dead player {} back to choosing class",
        identity
    );
    ...
    if let Some(mut account) = ctx.db.account().identity().find(&identity) {
        if account.state == AccountState::Dead {
            ...
            ctx.db.account().identity().update(account);
            log::info!(
                "Account {} transitioned from Dead to ChoosingClass",
                identity
            );
        } else {
            log::warn!(
                "Account {} was not in Dead state when transition timer fired. Current state: {:?}",
                identity,
                account.state
            );
        }
        ...
        log::warn!(
            "Account {} not found when dead transition timer fired",
            identity
        );
    }
}
...
pub fn transition_winner_to_curse_cutscene(ctx: &ReducerContext, timer: WinnerTransitionTimer) {
    if ctx.sender() != ctx.identity() {
        panic!(
            "TransitionWinnerToCurseCutscene may not be invoked by clients, only via scheduling."
        );
    }
    ...
    if let Some(mut account) = ctx.db.account().identity().find(&identity) {
        if account.state == AccountState::Winner {
            ...
            ctx.db.account().identity().update(account);
            log::info!(
                "Account {} transitioned from Winner to CurseCutscene",
                identity
            );
            ...
            ctx.db
                ...
                .insert(CurseTransitionTimer {
                    scheduled_id: 0,
                    scheduled_at: ScheduleAt::Time(
                        ctx.timestamp + Duration::from_millis(CURSE_CUTSCENE_DURATION_MS),
                    ),
                    identity,
                });

            log::info!(
                "Scheduled transition from CurseCutscene to ChoosingClass for account {} in {} ms",
                identity,
                CURSE_CUTSCENE_DURATION_MS
            );
        } else {
        ...
        log::warn!(
            "Account {} not found when winner transition timer fired",
            identity
        );
    }
}
...
pub fn transition_curse_to_choosing_class(ctx: &ReducerContext, timer: CurseTransitionTimer) {
    if ctx.sender() != ctx.identity() {
        panic!(
            "TransitionCurseToChoosingClass may not be invoked by clients, only via scheduling."
        );
    }

    let identity = timer.identity;
    log::info!(
        "Transitioning player {} from curse cutscene back to choosing class",
        identity
    );
    ...
    if let Some(mut account) = ctx.db.account().identity().find(&identity) {
        if account.state == AccountState::CurseCutscene {
            ...
            ctx.db.account().identity().update(account);
            log::info!(
                "Account {} transitioned from CurseCutscene to ChoosingClass",
                identity
            );
        } else {
        ...
        log::warn!(
            "Account {} not found when curse transition timer fired",
            identity
        );
    }
}
...
pub fn transition_player_to_dead_state(ctx: &ReducerContext, player_id: u32) {
    ...
    if let Some(account) = found_account {
        ...
        log::info!(
            "Account {} transitioned to Dead state and reset player ID to 0",
            identity
        );
        ...
        ctx.db
            ...
            .insert(DeadPlayerTransitionTimer {
                scheduled_id: 0,
                scheduled_at: ScheduleAt::Time(
                    ctx.timestamp + Duration::from_millis(DEAD_TRANSITION_DELAY_MS),
                ),
                identity,
            });

        log::info!(
            "Scheduled transition from Dead to ChoosingClass for account {} in {} ms",
            identity,
            DEAD_TRANSITION_DELAY_MS
        );
    } else {
        log::warn!(
            "Could not find account for player {} when transitioning to dead state",
            player_id
        );
    }
}
...
pub fn transition_player_to_winner_state(ctx: &ReducerContext, player_id: u32) {
    ...
    if let Some(account) = found_account {
        ...
        log::info!(
            "Account {} transitioned to Winner state and reset player ID to 0",
            identity
        );
        ...
        ctx.db
            ...
            .insert(WinnerTransitionTimer {
                scheduled_id: 0,
                scheduled_at: ScheduleAt::Time(
                    ctx.timestamp + Duration::from_millis(WINNER_TRANSITION_DELAY_MS),
                ),
                identity,
            });

        log::info!(
            "Scheduled transition from Winner to ChoosingClass for account {} in {} ms",
            identity,
            WINNER_TRANSITION_DELAY_MS
        );
    } else {
        log::warn!(
            "Could not find account for player {} when transitioning to winner state",
            player_id
        );
    }
}
...
pub fn require_admin_access(ctx: &ReducerContext, function_name: &str) {
    if !is_admin_caller(ctx) {
        ...
        panic!(
            "{}: Access denied. Only AdminXanadar can use debug commands. Caller: {} ({})",
...
pub fn debug_set_super_health(ctx: &ReducerContext) {
    ...
    if let Some(account) = ctx.db.account().identity().find(&caller_identity) {
        if account.current_player_id == 0 {
            log::warn!(
                "Admin {} has no active player to set super health",
                account.name
            );
            return;
        }
        ...
        if let Some(mut player) = ctx.db.player().player_id().find(&account.current_player_id) {
            ...
            log::info!(
                "Admin {} set super health: Player {} HP changed from {}/{} to 10000/10000",
                account.name,
                account.current_player_id,
                old_hp,
                old_max_hp
            ...
            log::warn!(
                "Admin {} player {} not found for super health",
                account.name,
                account.current_player_id
            );
        }
