use crate::{
    account, bestiary, config, gems_def, monsters, monsters_boid, player, DbVector2, GemLevel,
    MonsterType,
};
...
#[table(accessor = loot_capsules, scheduled(spawn_loot_capsule), public)]
...
#[table(accessor = guaranteed_void_chest_spawns, scheduled(spawn_guaranteed_void_chest), public)]
...
pub fn spawn_guaranteed_void_chest(ctx: &ReducerContext, spawn: GuaranteedVoidChestSpawn) {
    ...
    log::info!(
        "Guaranteed VoidChest spawn triggered for chest index {}!",
        spawn.chest_index
    );

    // Check for curse-based skipping
    if spawn.chest_index == 0
        && crate::curses_defs::is_curse_active(ctx, crate::curses_defs::CurseType::OneLessVoidChest)
    {
        log::info!("OneLessVoidChest curse active - skipping first void chest spawn (index 0)");
        return;
    }

    if spawn.chest_index == 1
        && crate::curses_defs::is_curse_active(
            ctx,
            crate::curses_defs::CurseType::OneLessVoidChestSecond,
        )
    {
        log::info!(
            "OneLessVoidChestSecond curse active - skipping second void chest spawn (index 1)"
        );
        return;
    }
    ...
    let bestiary_entry_opt = ctx
        ...
        .bestiary()
        .bestiary_id()
        .find(&(crate::MonsterType::VoidChest as u32));
...
pub fn spawn_debug_loot_capsule(ctx: &ReducerContext) {
    ...
    if player_opt.is_none() {
        log::error!(
            "spawn_debug_loot_capsule: Player {} not found",
...
pub fn spawn_loot_capsule_in_radius(
    ...
    let capsule = ctx.db.loot_capsules().insert(LootCapsules {
        ...
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(CAPSULE_FLIGHT_DURATION_MS),
        ),
    });
...
pub fn trigger_void_chest_damage_pinata(ctx: &ReducerContext, chest_position: DbVector2) {
    ...
    if roll <= VOID_CHEST_DAMAGE_CAPSULE_CHANCE {
        ...
        log::info!(
            "VoidChest damage pinata triggered at ({:.1}, {:.1})",
            chest_position.x,
            chest_position.y
        );
    }
...
pub fn trigger_void_chest_death_pinata(ctx: &ReducerContext, chest_position: DbVector2) {
    ...
    log::info!(
        "VoidChest death pinata triggered! Spawning {} capsules around ({:.1}, {:.1})",
        VOID_CHEST_DEATH_CAPSULE_COUNT,
        chest_position.x,
        chest_position.y
    );
    ...
    log::info!(
        "VoidChest death pinata complete - {} capsules spawned (with guaranteed booster pack)",
        VOID_CHEST_DEATH_CAPSULE_COUNT
    );
}
...
pub fn schedule_guaranteed_void_chest_spawns(ctx: &ReducerContext) {
    ...
    ctx.db
        ...
        .insert(GuaranteedVoidChestSpawn {
            scheduled_id: 0,
