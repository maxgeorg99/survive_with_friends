...
use crate::{
    bestiary, config, game_state, get_world_cell_from_position, monsters, monsters_boid, player,
    spatial_hash_collision_checker, DbVector2, MonsterAttackType, MonsterSpawners, MonsterType,
    MonsterVariant, DELTA_TIME, WORLD_CELL_BIT_SHIFT, WORLD_CELL_MASK, WORLD_GRID_HEIGHT,
    WORLD_GRID_WIDTH, WORLD_SIZE,
};
use spacetimedb::{
...
const AGNA_RITUAL_CANDLE_SPAWN_INTERVAL_MS: u64 =
    AGNA_RITUAL_MATCH_DURATION_MS / (AGNA_RITUAL_CANDLE_COUNT as u64); // ~307ms between candle spawns
...
#[table(accessor = boss_agna_last_patterns, public)]
...
#[table(accessor = agna_flamethrower_scheduler, scheduled(trigger_agna_flamethrower_attack), public)]
...
#[table(accessor = agna_magic_circles, public)]
...
#[table(accessor = agna_fire_orb_scheduler, scheduled(trigger_agna_fire_orb_attack), public)]
...
#[table(accessor = agna_delayed_orb_scheduler, scheduled(spawn_delayed_agna_fire_orb), public)]
...
#[table(accessor = agna_candle_spawns, public)]
#[derive(Clone)]
...
#[table(accessor = agna_candle_scheduler, scheduled(spawn_agna_candle), public)]
...
#[table(accessor = agna_candle_bolt_scheduler, scheduled(trigger_agna_candle_bolt), public)]
...
#[table(accessor = agna_ritual_completion_check, scheduled(check_agna_ritual_completion), public)]
...
#[table(accessor = agna_summoning_circle_spawner, scheduled(spawn_agna_summoning_circle), public)]
...
#[table(accessor = agna_target_switch_scheduler, scheduled(trigger_agna_target_switch), public)]
...
#[table(accessor = agna_phase2_flamethrower_scheduler, scheduled(trigger_agna_phase2_flamethrower_attack), public)]
...
pub fn handle_agna_attack_movement(_ctx: &ReducerContext, attack: &mut crate::ActiveMonsterAttack) {
    match attack.monster_attack_type {
        MonsterAttackType::AgnaFlamethrowerJet => {
            ...
            let radius_range =
                AGNA_FLAMETHROWER_JET_FINAL_RADIUS - AGNA_FLAMETHROWER_JET_INITIAL_RADIUS;
        ...
        MonsterAttackType::AgnaPhase2FlameJet => {
            ...
            // Apply eased progress to radius growth from initial to final
            let radius_range =
                AGNA_PHASE2_FLAME_JET_FINAL_RADIUS - AGNA_PHASE2_FLAME_JET_INITIAL_RADIUS;
...
#[reducer]
pub fn trigger_agna_flamethrower_attack(
    ctx: &ReducerContext,
    scheduler: AgnaFlamethrowerScheduler,
) {
    if ctx.sender() != ctx.identity() {
        panic!(
            "trigger_agna_flamethrower_attack may not be invoked by clients, only via scheduling."
        );
    }
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Agna boss {} no longer exists, stopping flamethrower attacks",
                scheduler.boss_monster_id
            );
            return;
        }
    };
    ...
    if boss.bestiary_id != MonsterType::BossAgnaPhase1
        && boss.bestiary_id != MonsterType::BossAgnaPhase2
    {
        log::info!(
            "Boss {} is not Agna, stopping flamethrower attacks",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    if boss.ai_state != crate::monster_ai_defs::AIState::BossAgnaFlamethrower {
        log::info!(
            "Agna boss {} no longer in flamethrower state, stopping attacks",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    let final_target_player_id = if current_target != scheduled_target {
        log::info!(
            "Agna boss {} target changed from {} to {}, updating flamethrower target",
            scheduler.boss_monster_id,
            scheduled_target,
            current_target
        );
    ...
    let target_player = match target_player_opt {
        ...
        None => {
            log::info!(
                "Target player {} no longer exists, continuing flamethrower with random target",
                final_target_player_id
            );
            // Try to find a random player instead
            match find_random_player(ctx) {
                Some(player_id) => match ctx.db.player().player_id().find(&player_id) {
                    Some(player) => player,
                    None => {
    ...
    let boss_position = match boss_boid_opt {
        ...
        None => {
            log::info!(
                "Boss {} boid not found, stopping flamethrower attacks",
                boss.monster_id
            );
            return;
        }
    ...
    let jet_attack = crate::ActiveMonsterAttack {
        ...
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(AGNA_FLAMETHROWER_JET_DURATION_MS),
        ),
    ...
    log::info!(
        "Agna boss {} fired flamethrower jet towards ({}, {})",
        scheduler.boss_monster_id,
        target_position.x,
        target_position.y
    );
...
fn schedule_next_agna_flamethrower_attack(
    ctx: &ReducerContext,
    boss_monster_id: u32,
    ...
    ctx.db
        ...
        .insert(AgnaFlamethrowerScheduler {
            scheduled_id: 0,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(AGNA_FLAMETHROWER_JET_INTERVAL_MS),
            ),
            boss_monster_id,
            target_player_id,
        });
...
pub fn start_agna_flamethrower_attacks(
    ctx: &ReducerContext,
    boss_monster_id: u32,
    target_player_id: u32,
) {
    log::info!(
        "Starting flamethrower attacks for Agna boss {} targeting player {}",
        boss_monster_id,
        target_player_id
    );
...
pub fn cleanup_agna_flamethrower_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    log::info!(
        "Cleaning up flamethrower schedules for Agna boss {}",
        boss_monster_id
    );

    // Find and delete all flamethrower schedulers for this boss
    let schedulers_to_delete: Vec<_> = ctx
        .db
        .agna_flamethrower_scheduler()
        .iter()
    ...
    for scheduler in schedulers_to_delete {
        ctx.db
            .agna_flamethrower_scheduler()
            .scheduled_id()
            .delete(&scheduler.scheduled_id);
        log::info!(
            "Deleted flamethrower scheduler {} for boss {}",
...
pub fn apply_agna_flamethrower_speed_boost(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Applying flamethrower speed boost to Agna boss {}",
        monster.monster_id
    );
...
pub fn execute_boss_agna_flamethrower_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Agna boss {} entering flamethrower state",
        monster.monster_id
    );
    ...
    let target_player_id = match find_random_player(ctx) {
        ...
        None => {
            ...
            schedule_state_change(
                ctx,
                monster.monster_id,
                crate::monster_ai_defs::AIState::BossAgnaIdle,
                1000,
            );
            return;
        }
    ...
    log::info!(
        "Agna boss {} selected new random target player {} for flamethrower attack and chase",
        monster.monster_id,
        target_player_id
    );
...
pub fn schedule_random_boss_agna_pattern(ctx: &ReducerContext, monster_id: u32, delay_ms: u64) {
    log::info!(
        "Scheduling random Agna pattern for monster {} in {}ms",
        monster_id,
        delay_ms
    );
...
fn increase_monster_speed(ctx: &ReducerContext, monster: &crate::Monsters, multiplier: f32) {
    ...
    log::info!(
        "Agna monster {} speed increased from {} to {} (base: {}, multiplier: {})",
        monster.monster_id,
        monster.speed,
        target_speed,
        base_speed,
...
fn schedule_state_change(
    ctx: &ReducerContext,
    monster_id: u32,
    target_state: crate::monster_ai_defs::AIState,
    delay_ms: u64,
) {
    ctx.db
        .monster_state_changes()
        .insert(crate::monster_ai_defs::MonsterStateChange {
            scheduled_id: 0,
    ...
    log::info!(
        "Scheduled state change for Agna monster {} to {:?} in {}ms",
        monster_id,
        target_state,
        delay_ms
    );
...
pub fn cleanup_agna_ai_schedules(ctx: &ReducerContext, monster_id: u32) {
    log::info!(
        "Cleaning up all Agna AI schedules for monster {}",
        monster_id
    );
...
pub fn trigger_agna_fire_orb_attack(ctx: &ReducerContext, scheduler: AgnaFireOrbScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Agna boss {} no longer exists, stopping fire orb attacks",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    if boss.ai_state != crate::monster_ai_defs::AIState::BossAgnaMagicCircle {
        log::info!(
            "Agna boss {} no longer in magic circle state, stopping fire orb attacks",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    let target_player_opt = ctx
        .db
        .player()
        .player_id()
        .find(&scheduler.target_player_id);
    let target_player = match target_player_opt {
        Some(player) => player,
        None => {
            log::info!(
                "Target player {} no longer exists for fire orb attack",
                scheduler.target_player_id
            );
            return;
        }
    };
    ...
    if player_circles.is_empty() {
        log::info!(
            "No magic circles found for player {}, cannot spawn fire orb",
            scheduler.target_player_id
        );
        return;
    }
    ...
    let orb_spawn_telegraph = crate::ActiveMonsterAttack {
        active_monster_attack_id: 0, // Will be auto-assigned
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(AGNA_ORB_SPAWN_DURATION_MS),
        ),
    ...
    ctx.db
        ...
        .insert(AgnaDelayedOrbScheduler {
            scheduled_id: 0,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(AGNA_ORB_SPAWN_TELEGRAPH_MS),
            ),
    ...
    log::info!(
        "Agna boss {} spawned orb spawn telegraph from circle {} targeting player {}",
        scheduler.boss_monster_id,
        chosen_circle.circle_index,
        scheduler.target_player_id
    );
...
pub fn spawn_delayed_agna_fire_orb(ctx: &ReducerContext, scheduler: AgnaDelayedOrbScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Agna boss {} no longer exists, cancelling delayed fire orb",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    if boss.ai_state != crate::monster_ai_defs::AIState::BossAgnaMagicCircle {
        log::info!(
            "Agna boss {} no longer in magic circle state, cancelling delayed fire orb",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    let target_player = match target_player_opt {
        ...
        None => {
            log::info!(
                "Target player {} no longer exists, cancelling delayed fire orb",
                scheduler.target_player_id
            );
            return;
        }
    ...
    let fire_orb_attack = crate::ActiveMonsterAttack {
        active_monster_attack_id: 0, // Will be auto-assigned
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(AGNA_FIRE_ORB_DURATION_MS),
        ),
    ...
    log::info!(
        "Agna boss {} spawned delayed fire orb from circle {} targeting player {}",
        scheduler.boss_monster_id,
        scheduler.circle_index,
        scheduler.target_player_id
    );
}

// Calculate the current position of a magic circle
fn calculate_magic_circle_position(
    player_position: &DbVector2,
    circle: &AgnaMagicCircle,
) -> DbVector2 {
...
fn schedule_next_agna_fire_orb_attack(
    ctx: &ReducerContext,
    boss_monster_id: u32,
    target_player_id: u32,
) {
    ctx.db
        ...
        .insert(AgnaFireOrbScheduler {
            scheduled_id: 0,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(AGNA_FIRE_ORB_SPAWN_INTERVAL_MS),
            ),
            boss_monster_id,
            target_player_id,
        });
...
pub fn spawn_agna_magic_circles(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    let players: Vec<_> = ctx.db.player().iter().collect();
    log::info!(
        "Found {} players to spawn magic circles around",
        players.len()
    );
    ...
    log::info!(
        "Total magic circles in database after spawning: {}",
        total_circles
    );
}
...
fn spawn_magic_circles_for_player(ctx: &ReducerContext, boss_monster_id: u32, player_id: u32) {
    ...
    for i in 0..AGNA_MAGIC_CIRCLES_PER_PLAYER {
        let initial_rotation = (i as f32)
            * (2.0 * std::f32::consts::PI / AGNA_MAGIC_CIRCLES_PER_PLAYER as f32)
            + base_angle_offset;
    ...
    log::info!(
        "Spawned {} magic circles around player {} at position ({}, {})",
        AGNA_MAGIC_CIRCLES_PER_PLAYER,
        player_id,
        player_position.x,
        player_position.y
    );
...
pub fn start_agna_fire_orb_attacks(
    ctx: &ReducerContext,
    boss_monster_id: u32,
    target_player_id: u32,
) {
    log::info!(
        "Starting fire orb attacks for Agna boss {} targeting player {}",
        boss_monster_id,
        target_player_id
    );
...
pub fn update_agna_magic_circles(ctx: &ReducerContext, cache: &crate::collision::CollisionCache) {
    ...
    for circle in ctx.db.agna_magic_circles().iter() {
        ...
        let player_cache_idx = match cache
            .player
            .player_id_to_cache_index
            .get(&updated_circle.target_player_id)
        {
            Some(&idx) => idx as usize,
            None => {
                log::warn!(
                    "Player {} not found when updating magic circle position",
                    updated_circle.target_player_id
                );
                continue;
            }
        };
        ...
        updated_circle.position =
            calculate_magic_circle_position(&player_position, &updated_circle);

        ctx.db
            .agna_magic_circles()
            .circle_id()
            .update(updated_circle);
...
pub fn cleanup_agna_magic_circle_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    log::info!(
        "Cleaning up magic circle schedules for Agna boss {}",
        boss_monster_id
    );

    // Delete all magic circles for this boss
    let circles_to_delete: Vec<_> = ctx
        .db
        .agna_magic_circles()
        .iter()
    ...
    let delayed_schedulers_to_delete: Vec<_> = ctx
        .db
        .agna_delayed_orb_scheduler()
        .iter()
        .filter(|scheduler| scheduler.boss_monster_id == boss_monster_id)
...
pub fn execute_boss_agna_magic_circle_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Agna boss {} entering magic circle state",
        monster.monster_id
    );
    ...
    log::info!(
        "About to spawn magic circles for Agna boss {}",
        monster.monster_id
    );
    spawn_agna_magic_circles(ctx, monster.monster_id);
    log::info!(
        "Finished spawning magic circles for Agna boss {}",
        monster.monster_id
    );
...
pub fn execute_boss_agna_ritual_match_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Starting Agna ritual match phase for boss {}",
        monster.monster_id
    );
    ...
    let boid = match boid_opt {
        ...
        None => {
            log::info!(
                "Boid {} not found for ritual candle spawn creation",
                monster.monster_id
            );
            return;
        }
    ...
    schedule_state_change(
        ctx,
        monster.monster_id,
        crate::monster_ai_defs::AIState::BossAgnaRitualWick,
        AGNA_RITUAL_MATCH_DURATION_MS,
    );
}
...
pub fn execute_boss_agna_ritual_wick_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Starting Agna ritual wick phase for boss {} - candles already spawned during match phase",
        monster.monster_id
    );
    ...
    ctx.db
        ...
        .insert(AgnaRitualCompletionCheck {
            scheduled_id: 0,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(AGNA_RITUAL_WICK_DURATION_MS),
            ),
...
pub fn execute_boss_agna_ritual_failed_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Agna ritual failed for boss {} - now vulnerable",
        monster.monster_id
    );
    ...
    schedule_state_change(
        ...
        monster.monster_id,
        crate::monster_ai_defs::AIState::BossAgnaIdle,
        AGNA_RITUAL_FAILED_DURATION_MS,
    );
}
...
pub fn execute_boss_agna_ritual_complete_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    log::info!(
        "Agna ritual completed for boss {} - dealing damage to all players",
        monster.monster_id
    );
    ...
    log::info!(
        "Candles from ritual remain active and continue attacking as ongoing threats for boss {}",
        monster.monster_id
    );
    ...
    schedule_state_change(
        ...
        monster.monster_id,
        crate::monster_ai_defs::AIState::BossAgnaIdle,
        AGNA_RITUAL_COMPLETE_DURATION_MS,
    ); // 2 seconds
}

// Helper function to create candle spawn positions in a circle
fn create_ritual_candle_spawns(
    ctx: &ReducerContext,
    boss_monster_id: u32,
    boss_position: &DbVector2,
) {
    log::info!(
        "Creating {} candle spawns around boss {}",
        AGNA_RITUAL_CANDLE_COUNT,
        boss_monster_id
    );
    ...
    for i in 0..AGNA_RITUAL_CANDLE_COUNT {
        ...
        log::info!(
            "Created candle spawn {} at ({}, {}) with 90° rotation (12 o'clock start)",
            i,
            spawn_position.x,
            spawn_position.y
        );
...
fn schedule_progressive_candle_spawns(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    for (index, spawn) in spawns.iter().enumerate() {
        ...
        log::info!(
            "Scheduled candle {} (index {}) to spawn in {}ms at 12 o'clock + {} positions",
            spawn.spawn_id,
            spawn.candle_index,
            delay_ms,
            spawn.candle_index
...
fn spawn_all_ritual_candles(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    for spawn in spawns {
        ...
        let bestiary_entry = ctx
            ...
            .bestiary()
            .bestiary_id()
            .find(&(MonsterType::AgnaCandle as u32))
        ...
        log::info!(
            "Spawned Agna candle {} at position ({}, {})",
            candle_monster_id,
            spawn.position.x,
            spawn.position.y
        );
...
fn start_candle_bolt_attacks(ctx: &ReducerContext, candle_monster_id: u32) {
    if let Some(target_player_id) = find_random_player(ctx) {
        ...
        ctx.db
            ...
            .insert(AgnaCandleBoltScheduler {
                scheduled_id: 0,
...
fn schedule_next_candle_bolt(ctx: &ReducerContext, candle_monster_id: u32, target_player_id: u32) {
    ...
    let delay_ms = AGNA_CANDLE_BOLT_INTERVAL_MS - variance_ms
        + (rng.gen::<f32>() * (variance_ms * 2) as f32) as u64;
...
fn fire_candle_bolt(ctx: &ReducerContext, candle: &crate::Monsters, target_player_id: u32) {
    ...
    let candle_boid = match candle_boid_opt {
        ...
        None => {
            log::info!(
                "Candle {} has no boid data, cannot fire bolt",
                candle.monster_id
            );
            return;
        }
...
fn cleanup_agna_candle_spawns(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    for spawn in spawns_to_delete {
        ...
        if spawn.candle_monster_id != 0 {
            if let Some(_candle) = ctx
                .db
                .monsters()
                .monster_id()
                .find(&spawn.candle_monster_id)
            {
                ctx.db
                    .monsters()
                    .monster_id()
                    .delete(&spawn.candle_monster_id);
...
fn cleanup_agna_candle_spawn_indicators(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    log::info!(
        "Cleaned up candle spawn indicators for boss {} (candles remain alive)",
        boss_monster_id
    );
}
...
fn cleanup_agna_candle_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    let bolt_schedulers: Vec<_> = ctx.db.agna_candle_bolt_scheduler().iter().collect();
    ...
    for scheduler in bolt_schedulers {
        ...
        if let Some(_candle) = ctx
            ...
            .monsters()
            .monster_id()
            .find(&scheduler.candle_monster_id)
        {
            // Check if this candle is from our boss (via spawn records)
            let spawn_exists = ctx.db.agna_candle_spawns().iter().any(|spawn| {
                spawn.boss_monster_id == boss_monster_id
                    && spawn.candle_monster_id == scheduler.candle_monster_id
            });
...
fn cleanup_agna_ritual_completion_checks(ctx: &ReducerContext, boss_monster_id: u32) {
    let checks_to_delete: Vec<_> = ctx
        .db
        .agna_ritual_completion_check()
        .iter()
    ...
    for check in checks_to_delete {
        ctx.db
            .agna_ritual_completion_check()
            .scheduled_id()
            .delete(&check.scheduled_id);
    }

    log::info!(
        "Cleaned up ritual completion checks for boss {}",
        boss_monster_id
    );
}
...
pub fn spawn_agna_candle(ctx: &ReducerContext, scheduler: AgnaCandleScheduler) {
    ...
    if boss_opt.is_none() {
        log::info!(
            "Agna boss {} no longer exists, cancelling candle spawn",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    log::info!(
        "Spawned Agna candle {} at position ({}, {}) during match phase",
        candle_monster.monster_id,
        spawn.position.x,
        spawn.position.y
    );
...
pub fn trigger_agna_candle_bolt(ctx: &ReducerContext, scheduler: AgnaCandleBoltScheduler) {
    ...
    let candle = match candle_opt {
        ...
        None => {
            log::info!(
                "Candle monster {} no longer exists",
                scheduler.candle_monster_id
            );
            return;
        }
    };
    ...
    let target = match target_opt {
        ...
        None => {
            ...
            if let Some(random_player_id) = find_random_player(ctx) {
                log::info!(
                    "Target player {} no longer exists, switching to {}",
                    scheduler.target_player_id,
                    random_player_id
                );
...
pub fn check_agna_ritual_completion(ctx: &ReducerContext, checker: AgnaRitualCompletionCheck) {
    ...
    let boss_opt = ctx
        ...
        .monsters()
        .monster_id()
        .find(&checker.boss_monster_id);
    let boss = match boss_opt {
        Some(monster) => monster,
        None => {
            log::info!(
                "Boss {} no longer exists for ritual completion check",
                checker.boss_monster_id
            );
            return;
        }
    };
    ...
    if boss.ai_state != crate::monster_ai_defs::AIState::BossAgnaRitualWick {
        log::info!(
            "Boss {} no longer in ritual wick state, skipping completion check",
            checker.boss_monster_id
        );
        return;
    }
    ...
    log::info!(
        "Ritual completion check for boss {}: {} candles alive",
        checker.boss_monster_id,
        living_candles
    );
    ...
    if living_candles == 0 {
        ...
        log::info!(
            "All candles destroyed! Ritual failed for boss {}",
            checker.boss_monster_id
        );
        ...
        log::info!(
            "Ritual completed! {} candles survived for boss {}",
            living_candles,
            checker.boss_monster_id
        );
...
fn count_living_candles(ctx: &ReducerContext, boss_monster_id: u32) -> u32 {
    ...
    log::info!(
        "Counting candles: found {} living AgnaCandle monsters (boss_monster_id {} ignored)",
        living_count,
        boss_monster_id
    );
...
pub fn process_agna_ritual_complete_damage(ctx: &ReducerContext) {
    ...
    let ritual_complete_bosses: Vec<_> = ctx
        .db
        .monsters()
        .iter()
        .filter(|monster| {
    ...
    for player in ctx.db.player().iter() {
        if player.hp > 0.0 {
            ...
            log::info!(
                "Ritual complete: Player {} took {} damage (HP: {} -> {})",
                player_id,
                total_damage,
                old_hp,
                new_hp
    ...
    if !ritual_complete_bosses.is_empty() {
        log::info!(
            "Agna ritual complete damage: {} damage dealt to all living players by {} boss(es)",
            total_damage,
            ritual_complete_bosses.len()
        );
    }
}
...
pub fn spawn_agna_summoning_circle(ctx: &ReducerContext, spawner: AgnaSummoningCircleSpawner) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Phase 2 Agna boss {} no longer exists, stopping summoning circle spawning",
                spawner.boss_monster_id
            );
            return;
        }
    };
    ...
    if boss.bestiary_id != MonsterType::BossAgnaPhase2 {
        log::info!(
            "Boss {} is not Agna Phase 2, stopping summoning circle spawning",
            spawner.boss_monster_id
        );
        return;
    }
    ...
    if player_count == 0 {
        ...
        schedule_next_agna_summoning_circle(
            ctx,
            spawner.boss_monster_id,
    ...
    spawn_position.x = spawn_position
        .x
        .clamp(100.0, config.world_size as f32 - 100.0);
    spawn_position.y = spawn_position
        .y
        .clamp(100.0, config.world_size as f32 - 100.0);

    log::info!(
        "Spawning Agna summoning circle at position ({:.1}, {:.1}) near player {}",
        spawn_position.x,
        spawn_position.y,
        target_player.name
    );
    ...
    let ground_flame_attack = crate::ActiveMonsterAttack {
        active_monster_attack_id: 0, // Will be auto-assigned
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(AGNA_GROUND_FLAME_DURATION_MS),
        ),
        position: spawn_position,
    ...
    log::info!(
        "Spawned AgnaGroundFlame at summoning circle position ({:.1}, {:.1})",
        spawn_position.x,
        spawn_position.y
    );
...
fn schedule_next_agna_summoning_circle(
    ...
    log::info!(
        "Scheduled next Agna summoning circle for boss {} in {}ms (reduced from {}ms)",
        boss_monster_id,
        next_interval_ms,
        current_interval_ms
    );
...
pub fn trigger_agna_target_switch(ctx: &ReducerContext, scheduler: AgnaTargetSwitchScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Boss {} no longer exists, stopping Agna target switching",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    if boss.bestiary_id != MonsterType::BossAgnaPhase2 {
        log::info!(
            "Boss {} is not Agna Phase 2, stopping target switching",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    if player_count == 0 {
        log::info!(
            "No players online for Agna boss {} target switch, stopping",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    let mut available_players: Vec<_> = players
        .into_iter()
    ...
    log::info!(
        "Agna boss {} switched target from player {} to player {} ({})",
        scheduler.boss_monster_id,
        current_target_id,
...
fn schedule_next_agna_target_switch(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    ctx.db
        ...
        .insert(AgnaTargetSwitchScheduler {
            scheduled_id: 0,
            boss_monster_id,
            scheduled_at: ScheduleAt::Time(ctx.timestamp + Duration::from_millis(next_interval_ms)),
        });
    ...
    log::info!(
        "Scheduled next Agna target switch for boss {} in {}ms",
        boss_monster_id,
        next_interval_ms
    );
}
...
#[reducer]
pub fn trigger_agna_phase2_flamethrower_attack(
    ctx: &ReducerContext,
    scheduler: AgnaPhase2FlamethrowerScheduler,
) {
    if ctx.sender() != ctx.identity() {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Agna Phase 2 boss {} no longer exists, stopping flamethrower attacks",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    if boss.bestiary_id != MonsterType::BossAgnaPhase2 {
        log::info!(
            "Boss {} is not Agna Phase 2, stopping flamethrower attacks",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    let jet_attack = crate::ActiveMonsterAttack {
        active_monster_attack_id: 0, // Will be auto-assigned
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(AGNA_PHASE2_FLAME_JET_DURATION_MS),
        ),
        position: boss_position,
        direction: direction_vector,
    ...
    log::info!(
        "Agna Phase 2 boss {} fired flame jet at player {} ({})",
        scheduler.boss_monster_id,
        target_player_id,
        target_player.name
    );
...
fn schedule_next_agna_phase2_flamethrower_attack(ctx: &ReducerContext, boss_monster_id: u32) {
    ctx.db
        ...
        .insert(AgnaPhase2FlamethrowerScheduler {
            scheduled_id: 0,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(AGNA_PHASE2_FLAME_JET_INTERVAL_MS),
            ),
            boss_monster_id,
        });
}
...
pub fn start_agna_phase2_summoning_circles(ctx: &ReducerContext, boss_monster_id: u32) {
    log::info!(
        "Starting Agna Phase 2 summoning circles for boss {}",
        boss_monster_id
    );
    ...
    ctx.db
        ...
        .insert(AgnaSummoningCircleSpawner {
            ...
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(AGNA_PHASE2_SUMMONING_INITIAL_INTERVAL_MS),
            ),
        });

    log::info!(
        "Agna Phase 2 summoning circles scheduled for boss {} (first circle in {}ms)",
        boss_monster_id,
        AGNA_PHASE2_SUMMONING_INITIAL_INTERVAL_MS
    );
}
...
pub fn start_agna_phase2_target_switching(ctx: &ReducerContext, boss_monster_id: u32) {
    ctx.db
        ...
        .insert(AgnaTargetSwitchScheduler {
            ...
            boss_monster_id,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(AGNA_PHASE2_TARGET_SWITCH_INITIAL_DELAY_MS),
            ),
        });

    log::info!(
        "Started Agna Phase 2 target switching for boss {} (first switch in {}ms)",
        boss_monster_id,
        AGNA_PHASE2_TARGET_SWITCH_INITIAL_DELAY_MS
    );
}
...
pub fn start_agna_phase2_flamethrower_attacks(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    log::info!(
        "Started Agna Phase 2 flamethrower attacks for boss {} (first attack in {}ms)",
        boss_monster_id,
        AGNA_PHASE2_FLAME_JET_INTERVAL_MS
    );
}
...
pub fn cleanup_agna_phase2_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    log::info!(
        "Cleaning up all Agna Phase 2 schedules for boss {}",
        boss_monster_id
    );

    // Cleanup summoning circle spawners
    let summoning_schedulers_to_delete: Vec<u64> = ctx
        .db
        .agna_summoning_circle_spawner()
        .boss_monster_id()
    ...
    let target_schedulers_to_delete: Vec<u64> = ctx
        .db
        .agna_target_switch_scheduler()
        .boss_monster_id()
        .filter(&boss_monster_id)
    ...
    let flamethrower_schedulers_to_delete: Vec<u64> = ctx
        .db
        .agna_phase2_flamethrower_scheduler()
        .boss_monster_id()
        .filter(&boss_monster_id)
    ...
    let active_attacks_to_delete: Vec<u64> = ctx
        .db
