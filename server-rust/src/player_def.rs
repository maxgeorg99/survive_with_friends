use crate::{
    account, collision, config, get_world_cell_from_position, monsters,
    spatial_hash_collision_checker, world, DbVector2, PlayerClass, DELTA_TIME,
    WORLD_CELL_BIT_SHIFT, WORLD_CELL_MASK, WORLD_GRID_HEIGHT, WORLD_GRID_WIDTH, WORLD_SIZE,
}; // Added world to imports
use spacetimedb::{rand::Rng, reducer, table, ReducerContext, ScheduleAt, Table};
...
pub fn is_position_safe(
    ...
    position: &DbVector2,
    radius: f32,
    collision_cache: &crate::CollisionCache,
) -> bool {
...
#[table(accessor = dead_players, public)]
...
pub fn set_player_waypoint(ctx: &ReducerContext, waypoint_x: f32, waypoint_y: f32) {
    ...
    let account = ctx.db.account().identity().find(&identity).expect(&format!(
        "SetPlayerWaypoint: Account {} does not exist.",
        identity
    ));
    ...
    let mut player = ctx
        ...
        .find(&player_id)
        .expect(&format!(
            "SetPlayerWaypoint: Player {} does not exist.",
            player_id
        ));
...
#[table(accessor = health_regen_scheduler, scheduled(process_health_regen))]
...
pub fn process_player_movement(
    ctx: &ReducerContext,
    tick_rate: u32,
    collision_cache: &mut collision::CollisionCache,
) {
    ...
    for player in ctx.db.player().iter() {
        ...
        if modified_player.has_waypoint {
            ...
            if distance < move_speed * DELTA_TIME {
                ...
                let normalized_direction =
                    DbVector2::new(direction_vector.x / distance, direction_vector.y / distance);
            ...
            let mut rng = ctx.rng();
            modified_player.waypoint.x = modified_player.radius
                + rng.gen_range(0.0..1.0) * (WORLD_SIZE as f32 - modified_player.radius * 2.0);
            modified_player.waypoint.y = modified_player.radius
                + rng.gen_range(0.0..1.0) * (WORLD_SIZE as f32 - modified_player.radius * 2.0);
        ...
        collision_cache.player.shield_count_player[cached_count_players] =
            modified_player.shield_count;
        collision_cache.player.pvp_player[cached_count_players] = modified_player.pvp;

        let grid_cell_key =
            get_world_cell_from_position(modified_player.position.x, modified_player.position.y)
                as usize;
        collision_cache.player.cell_player[cached_count_players] = grid_cell_key as i32;
        collision_cache.player.nexts_player[cached_count_players] =
            collision_cache.player.heads_player[grid_cell_key];
        collision_cache.player.heads_player[grid_cell_key] = cached_count_players as i32;

        collision_cache
            .player
            .player_id_to_cache_index
            .insert(modified_player.player_id, cached_count_players as u32);
...
pub fn process_player_monster_collisions_spatial_hash(
    ctx: &ReducerContext,
    collision_cache: &mut collision::CollisionCache,
) {
                ...
                while mid != -1 {
                    ...
                    if spatial_hash_collision_checker(px, py, pr, mx, my, mr) {
                        ...
                        if collision_cache.monster.can_deal_damage[mid_usize] {
                            collision_cache.player.damage_to_player[pid] +=
                                collision_cache.monster.atk_monster[mid_usize];
                        }
                    }
...
pub fn apply_negative_health_regen_damage(
    ctx: &ReducerContext,
    collision_cache: &mut collision::CollisionCache,
) {
    ...
    for pid in 0..collision_cache.player.cached_count_players as usize {
        if collision_cache.player.damage_to_player[pid] > 0.0 {
            crate::core_game::damage_player(
                ctx,
                collision_cache.player.keys_player[pid],
                collision_cache.player.damage_to_player[pid],
            );
        }
    }
...
pub fn set_player_pvp_mode(ctx: &ReducerContext, pvp_enabled: bool) {
    ...
    // Find the account for the caller
    let account = ctx.db.account().identity().find(&identity).expect(&format!(
        "SetPlayerPvpMode: Account {} does not exist.",
        identity
    ));
    ...
    let mut player = ctx
        ...
        .find(&player_id)
        .expect(&format!(
            "SetPlayerPvpMode: Player {} does not exist.",
            player_id
        ));
    ...
    log::info!(
        ...
        player.name,
        player_id,
        if pvp_enabled { "enabled" } else { "disabled" }
    );
