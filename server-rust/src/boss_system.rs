use crate::{
    active_attacks, attack_burst_cooldowns, bestiary, config, dead_players, entity, gems,
    monster_spawn_timer, monster_spawners, monsters, monsters_boid, player,
    player_scheduled_attacks, DbVector2, MonsterType,
};
...
#[table(accessor = boss_spawn_timer, scheduled(spawn_boss_phase_one), public)]
...
#[table(accessor = boss_phase_two_timer, scheduled(spawn_boss_phase_two_delayed), public)]
...
#[table(accessor = boss_selection, public)]
...
pub fn schedule_boss_spawn(ctx: &ReducerContext) {
    ...
    let boss_spawn_delay_ms = if crate::curses_defs::is_curse_active(
        ctx,
    ...
    if let Some(mut selection) = boss_selection_opt {
        ...
        log::info!(
            "Updated boss selection to randomly chosen {} ({:?})",
            boss_name,
            selected_boss_type
        );
    } else {
        ...
        log::info!(
            "Created new boss selection with randomly chosen {} ({:?})",
            boss_name,
            selected_boss_type
        );
    }
    ...
    log::info!(
        "Boss spawn timer scheduled - {} boss will spawn in 5 minutes",
        boss_name
    );
...
fn schedule_boss_spawning(ctx: &ReducerContext, position: DbVector2) {
    ...
    log::info!(
        "Spawning {} boss (type {:?}) at position ({}, {})",
        boss_name,
        boss_selection.boss_type,
        position.x,
        position.y
    );
    ...
    let _spawner_opt = ctx.db.monster_spawners().insert(crate::MonsterSpawners {
        ...
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(BOSS_SPAWN_VISUALIZATION_DELAY_MS),
        ),
    });
}
...
pub fn spawn_boss_phase_two(ctx: &ReducerContext, position: DbVector2) {
    ...
    log::info!(
        "Spawning {} Phase 2 boss at position ({}, {})",
        boss_name,
        position.x,
        position.y
    );
    ...
    let bestiary_entry = ctx
        ...
        .bestiary()
        .bestiary_id()
        .find(&(boss_monster_type.clone() as u32))
    ...
    if crate::curses_defs::is_curse_active(ctx, crate::curses_defs::CurseType::DeadlierBosses) {
        ...
        log::info!(
            "DeadlierBossesTwo curse applied to Phase 2 boss - HP: {}, ATK: {:.1}, Speed: {:.1}",
            final_hp,
            final_atk,
            final_speed
        );
    ...
    match boss_selection.boss_type {
        BossType::Ender => {
            ...
            log::info!(
                "Starting ChaosBall and VoidZone attacks for Phase 2 boss {}",
                monster.monster_id
            );
            ...
            log::info!(
                "Starting boss target switching for Phase 2 boss {}",
                monster.monster_id
            );
        ...
        BossType::Simon => {
            ...
            log::info!(
                "Starting Simon's special attacks for Phase 2 boss {}",
                monster.monster_id
            );
...
pub fn handle_boss_defeated(ctx: &ReducerContext) {
    ...
    log::info!(
        "{} players marked as True Survivors and transitioned to Winner state!",
        true_survivors_count
    );
...
pub fn spawn_boss_for_testing(ctx: &ReducerContext) {
    ...
    if let Some(timer) = timer_opt {
        log::info!(
            "DEVELOPER TEST: Found existing boss timer, updating to trigger in 5 seconds..."
        );
        ...
        ctx.db.boss_spawn_timer().insert(BossSpawnTimer {
            scheduled_id: 0,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(TEST_BOSS_SPAWN_DELAY_MS),
            ),
...
pub fn update_boss_monster_id(ctx: &ReducerContext, monster_id: u32) {
    ...
    if monster_opt.is_none() {
        log::info!(
            "UpdateBossMonsterID: Monster with ID {} not found!",
            monster_id
        );
        return;
    }
    ...
    {
        let boss_name = if monster.bestiary_id == MonsterType::BossEnderPhase1 {
            "Ender"
        } else if monster.bestiary_id == MonsterType::BossAgnaPhase1 {
            "Agna"
        } else {
            "Simon"
        };
        log::info!(
            "BOSS {} PHASE 1 CREATED: Updating game state with boss_monster_id={}",
            boss_name,
            monster_id
        );
        ...
        log::info!(
            "Game state updated with boss_monster_id={}, boss_active={}, boss_phase={}",
            monster_id,
...
pub fn schedule_boss_phase_two_spawn(ctx: &ReducerContext, position: DbVector2) {
    log::info!(
        "Scheduling boss phase 2 spawn at ({}, {}) with 1.5 second delay",
        position.x,
        position.y
    );
...
pub fn spawn_boss_phase_two_delayed(ctx: &ReducerContext, timer: BossPhase2Timer) {
    if ctx.sender() != ctx.identity() {
        panic!(
            "Reducer SpawnBossPhase2Delayed may not be invoked by clients, only via scheduling."
        );
    }
...
pub fn debug_set_boss_type(ctx: &ReducerContext, client_key: u32) {
    ...
    log::info!(
        "Debug: Setting boss type to {} ({:?}) from client key {}",
        boss_name,
        boss_type,
        client_key
    );
    ...
    if let Some(mut selection) = existing_selection {
        ...
        ctx.db.boss_selection().id().update(selection);
        log::info!(
            "Debug: Updated boss selection to {} ({:?})",
            boss_name,
            boss_type
        );
    } else {
        ...
        ctx.db
            .boss_selection()
            .insert(BossSelection { id: 0, boss_type });
        log::info!(
            "Debug: Created new boss selection: {} ({:?})",
            boss_name,
            boss_type
        );
    }
