...
use crate::{
    bestiary, config, get_world_cell_from_position, monsters, monsters_boid, player,
    spatial_hash_collision_checker, DbVector2, MonsterAttackType, MonsterSpawners, MonsterType,
    DELTA_TIME, WORLD_CELL_BIT_SHIFT, WORLD_CELL_MASK, WORLD_GRID_HEIGHT, WORLD_GRID_WIDTH,
};
use spacetimedb::{
...
#[table(accessor = boss_ender_last_patterns, public)]
...
#[table(accessor = ender_claw_spawner, scheduled(spawn_ender_claw_wave), public)]
...
#[table(accessor = ender_scythe_spawn_scheduler, scheduled(spawn_ender_scythe_spawns), public)]
...
#[table(accessor = ender_scythe_scheduler, scheduled(spawn_ender_scythes), public)]
...
#[table(accessor = ender_bolt_scheduler, scheduled(trigger_ender_bolt_attack), public)]
...
#[table(accessor = chaos_ball_scheduler, scheduled(trigger_chaos_ball_attack), public)]
...
#[table(accessor = void_zone_scheduler, scheduled(trigger_void_zone_attack), public)]
...
#[table(accessor = boss_target_switch_scheduler, scheduled(trigger_boss_target_switch), public)]
...
pub fn spawn_ender_claw_wave(ctx: &ReducerContext, spawner: EnderClawSpawner) {
    ...
    log::info!(
        "Spawning EnderClaw wave: {} EnderClaws for {} players (interval: {}ms)",
        player_count,
        player_count,
...
fn spawn_single_ender_claw(ctx: &ReducerContext, target_player: &crate::Player) {
    ...
    let bestiary_entry = ctx
        ...
        .bestiary()
        .bestiary_id()
        .find(&(MonsterType::EnderClaw as u32))
    ...
    log::info!(
        "Pre-spawned EnderClaw for player {} at position ({:.1}, {:.1})",
        target_player.name,
        spawn_position.x,
        spawn_position.y
    );
...
fn schedule_next_ender_claw_wave(
    ctx: &ReducerContext,
    boss_monster_id: u32,
    current_interval_ms: u64,
) {
    // Calculate next interval (reduce by 15% each wave, but don't go below minimum)
    let next_interval_ms = ((current_interval_ms as f32 * ENDER_CLAW_INTERVAL_REDUCTION_RATIO)
        as u64)
    ...
    log::info!(
        "Scheduled next EnderClaw wave for boss {} in {}ms (reduced from {}ms)",
        boss_monster_id,
        next_interval_ms,
        current_interval_ms
    );
...
pub fn start_ender_claw_spawning(ctx: &ReducerContext, boss_monster_id: u32) {
    log::info!(
        "Starting EnderClaw spawning for Phase 2 boss {}",
        boss_monster_id
    );
    ...
    ctx.db.ender_claw_spawner().insert(EnderClawSpawner {
        ...
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(ENDER_CLAW_INITIAL_INTERVAL_MS),
        ),
    });

    log::info!(
        "EnderClaw spawning scheduled for boss {} (first wave in {}ms)",
        boss_monster_id,
        ENDER_CLAW_INITIAL_INTERVAL_MS
    );
}
...
pub fn cleanup_ender_claw_spawning(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    let spawners_to_delete: Vec<u64> = ctx
        .db
        .ender_claw_spawner()
        .boss_monster_id()
    ...
    if spawner_count > 0 {
        log::info!(
            "Cleaned up {} EnderClaw spawners for boss {}",
            spawner_count,
            boss_monster_id
        );
    }
...
fn handle_ender_scythe_orbital_movement(
    ctx: &ReducerContext,
    attack: &mut crate::ActiveMonsterAttack,
) {
...
pub fn spawn_ender_scythe_spawns(ctx: &ReducerContext, scheduler: EnderScytheSpawnScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Boss {} no longer exists, cancelling EnderScytheSpawn spawning",
    ...
    let boss_position = match boid_opt {
        ...
        None => {
            log::info!(
                "Boss {} has no boid, cancelling EnderScytheSpawn spawning",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    log::info!(
        "Spawning EnderScytheSpawn pattern for boss {} at position ({}, {})",
        scheduler.boss_monster_id,
        boss_position.x,
        boss_position.y
    );
    ...
    for ring in 0..ENDER_SCYTHE_RINGS {
        ...
        for scythe_index in 0..scythe_count {
            ...
            let active_monster_attack =
                ctx.db
                    .active_monster_attacks()
                    .insert(crate::ActiveMonsterAttack {
                        active_monster_attack_id: 0,
                        scheduled_at: ScheduleAt::Time(
                            ctx.timestamp + Duration::from_millis(ENDER_SCYTHE_SPAWN_DURATION_MS),
                        ),
            ...
            log::info!(
                "Spawned EnderScytheSpawn {} at ({:.1}, {:.1}) angle {:.2} for boss {}",
...
pub fn spawn_ender_scythes(ctx: &ReducerContext, scheduler: EnderScytheScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Boss {} no longer exists, cancelling EnderScythe spawning",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    let boss_position = match boid_opt {
        ...
        None => {
            log::info!(
                "Boss {} has no boid, cancelling EnderScythe spawning",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    log::info!(
        "Spawning EnderScythe pattern for boss {} at position ({}, {})",
        scheduler.boss_monster_id,
        boss_position.x,
        boss_position.y
    );
    ...
    for ring in 0..ENDER_SCYTHE_RINGS {
        ...
        for scythe_index in 0..scythe_count {
            ...
            let active_monster_attack =
                ctx.db
                    ...
                    .insert(crate::ActiveMonsterAttack {
                        active_monster_attack_id: 0,
                        scheduled_at: ScheduleAt::Time(
                            ctx.timestamp + Duration::from_millis(scythe_duration_ms),
                        ),
                        position: spawn_position,
                        direction,
            ...
            log::info!(
                "Spawned EnderScythe {} at ({:.1}, {:.1}) angle {:.2} for boss {}",
                active_monster_attack.active_monster_attack_id,
                spawn_x,
                spawn_y,
                angle,
...
pub fn schedule_ender_scythe_attacks(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    log::info!(
        "Scheduling EnderScythe attack pattern for boss {} with base rotation {:.2}",
        boss_monster_id,
        base_rotation
    );
    ...
    ctx.db
        ...
        .insert(EnderScytheSpawnScheduler {
            scheduled_id: 0,
            boss_monster_id,
            base_rotation,
    ...
    ctx.db
        ...
        .insert(EnderScytheScheduler {
            scheduled_id: 0,
            boss_monster_id,
            base_rotation,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(1000 + ENDER_SCYTHE_SPAWN_DURATION_MS),
            ),
        });

    log::info!(
        "Scheduled EnderScythe pattern for boss {} (warning at 1s, attack at 3s)",
        boss_monster_id
    );
}
...
pub fn spawn_ender_bolt(ctx: &ReducerContext, spawn_position: DbVector2, target_player_id: u32) {
    ...
    let target_player = match target_player_opt {
        ...
        None => {
            log::error!(
                "Cannot spawn EnderBolt: target player {} not found",
                target_player_id
            );
            return;
        }
    ...
    let active_monster_attack =
        ctx.db
            ...
            .insert(crate::ActiveMonsterAttack {
                active_monster_attack_id: 0,
                scheduled_at: ScheduleAt::Time(
                    ctx.timestamp + Duration::from_millis(ENDER_BOLT_DURATION_MS),
                ),
                position: spawn_position,
    ...
    log::info!(
        "Spawned EnderBolt {} at ({}, {}) targeting player {} (expires in {}ms)",
        active_monster_attack.active_monster_attack_id,
        spawn_position.x,
        spawn_position.y,
        target_player_id,
        ENDER_BOLT_DURATION_MS
    );
}
...
pub fn trigger_ender_bolt_attack(ctx: &ReducerContext, scheduler: EnderBoltScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            ...
            log::info!(
                "Boss {} no longer exists, removing EnderBolt scheduler",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    if boss.ai_state != crate::monster_ai_defs::AIState::BossEnderDance {
        log::info!(
            "Boss {} no longer in dance mode, stopping EnderBolt attacks",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    let boss_position = match boid_opt {
        ...
        None => {
            ...
            log::info!(
                "Boss {} has no boid, removing EnderBolt scheduler",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    let target_player = match target_player {
        ...
        None => {
            ...
            log::info!(
                "No players found for boss {}, skipping EnderBolt attack",
                scheduler.boss_monster_id
            );
    ...
    log::info!(
        "Boss {} fired EnderBolt at player {} from position ({}, {})",
        scheduler.boss_monster_id,
...
fn schedule_next_ender_bolt_attack(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    log::info!(
        "Scheduled next EnderBolt for boss {} in {}ms ({} players active)",
        boss_monster_id,
        fire_interval_ms,
        player_count
    );
...
pub fn trigger_chaos_ball_attack(ctx: &ReducerContext, scheduler: ChaosBallScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Boss {} no longer exists, removing ChaosBall scheduler",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    if boss.bestiary_id != crate::MonsterType::BossEnderPhase2 {
        log::info!(
            "Boss {} is not Phase 2, stopping ChaosBall attacks",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    let boss_position = match boid_opt {
        ...
        None => {
            log::info!(
                "Boss {} has no boid, removing ChaosBall scheduler",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    let target_player = match target_player {
        ...
        None => {
            log::info!(
                "No players found for boss {}, skipping ChaosBall attack",
                scheduler.boss_monster_id
            );
    ...
    let active_monster_attack =
        ctx.db
            ...
            .insert(crate::ActiveMonsterAttack {
                active_monster_attack_id: 0,
                scheduled_at: ScheduleAt::Time(
                    ctx.timestamp + Duration::from_millis(CHAOS_BALL_DURATION_MS),
                ),
    ...
    log::info!(
        "Spawned ChaosBall {} targeting player {} ({}) from boss {}",
        active_monster_attack.active_monster_attack_id,
        target_player.name,
        target_player.player_id,
        scheduler.boss_monster_id
...
pub fn trigger_void_zone_attack(ctx: &ReducerContext, scheduler: VoidZoneScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Boss {} no longer exists, removing VoidZone scheduler",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    if boss.bestiary_id != crate::MonsterType::BossEnderPhase2 {
        log::info!(
            "Boss {} is not Phase 2, stopping VoidZone attacks",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    let boss_position = match boid_opt {
        ...
        None => {
            log::info!(
                "Boss {} has no boid, removing VoidZone scheduler",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    let active_monster_attack =
        ctx.db
            ...
            .insert(crate::ActiveMonsterAttack {
                active_monster_attack_id: 0,
                scheduled_at: ScheduleAt::Time(
                    ctx.timestamp + Duration::from_millis(VOID_ZONE_DURATION_MS),
                ),
                position: boss_position,
    ...
    log::info!(
        "Spawned VoidZone {} at position ({:.1}, {:.1}) from boss {}",
        active_monster_attack.active_monster_attack_id,
        boss_position.x,
        boss_position.y,
        scheduler.boss_monster_id
...
fn schedule_next_chaos_ball_attack(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    log::info!(
        "Scheduled next ChaosBall for boss {} in {}ms ({} players active)",
        boss_monster_id,
        fire_interval_ms,
        player_count
    );
...
fn schedule_next_void_zone_attack(ctx: &ReducerContext, boss_monster_id: u32) {
    ctx.db.void_zone_scheduler().insert(VoidZoneScheduler {
        ...
        boss_monster_id,
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(VOID_ZONE_INTERVAL_MS),
        ),
    });

    log::info!(
        "Scheduled next VoidZone for boss {} in {}ms",
        boss_monster_id,
        VOID_ZONE_INTERVAL_MS
    );
}
...
pub fn trigger_boss_target_switch(ctx: &ReducerContext, scheduler: BossTargetSwitchScheduler) {
    ...
    let boss = match boss_opt {
        ...
        None => {
            log::info!(
                "Boss {} no longer exists, stopping target switching",
                scheduler.boss_monster_id
            );
            return;
        }
    ...
    if boss.bestiary_id != crate::MonsterType::BossEnderPhase2 {
        log::info!(
            "Boss {} is not Phase 2, stopping target switching",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    if player_count == 0 {
        log::info!(
            "No players online for boss {} target switch, stopping",
            scheduler.boss_monster_id
        );
        return;
    }
    ...
    log::info!(
        "Boss {} switched target from player {} to player {} ({})",
        scheduler.boss_monster_id,
        current_target_id,
...
fn schedule_next_boss_target_switch(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    ctx.db
        ...
        .insert(BossTargetSwitchScheduler {
            scheduled_id: 0,
            boss_monster_id,
            scheduled_at: ScheduleAt::Time(ctx.timestamp + Duration::from_millis(next_interval_ms)),
        });
    ...
    log::info!(
        "Scheduled next boss target switch for boss {} in {}ms",
        boss_monster_id,
        next_interval_ms
    );
}
...
pub fn start_ender_bolt_attacks(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    ctx.db.ender_bolt_scheduler().insert(EnderBoltScheduler {
        ...
        boss_monster_id,
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(ENDER_BOLT_INITIAL_DELAY_MS),
        ),
    });

    log::info!(
        "Started EnderBolt attack schedule for boss {} (first attack in {}ms)",
        boss_monster_id,
        ENDER_BOLT_INITIAL_DELAY_MS
    );
}
...
pub fn start_chaos_ball_attacks(ctx: &ReducerContext, boss_monster_id: u32) {
    ctx.db.chaos_ball_scheduler().insert(ChaosBallScheduler {
        ...
        boss_monster_id,
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(CHAOS_BALL_INITIAL_DELAY_MS),
        ),
    });

    log::info!(
        "Started ChaosBall attack schedule for boss {} (first attack in {}ms)",
        boss_monster_id,
        CHAOS_BALL_INITIAL_DELAY_MS
    );
}
...
pub fn start_void_zone_attacks(ctx: &ReducerContext, boss_monster_id: u32) {
    ctx.db.void_zone_scheduler().insert(VoidZoneScheduler {
        ...
        boss_monster_id,
        scheduled_at: ScheduleAt::Time(
            ctx.timestamp + Duration::from_millis(VOID_ZONE_INITIAL_DELAY_MS),
        ),
    });

    log::info!(
        "Started VoidZone attack schedule for boss {} (first attack in {}ms)",
        boss_monster_id,
        VOID_ZONE_INITIAL_DELAY_MS
    );
}
...
pub fn start_boss_target_switching(ctx: &ReducerContext, boss_monster_id: u32) {
    ctx.db
        ...
        .insert(BossTargetSwitchScheduler {
            ...
            boss_monster_id,
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(BOSS_TARGET_SWITCH_INITIAL_DELAY_MS),
            ),
        });

    log::info!(
        "Started boss target switching for boss {} (first switch in {}ms)",
        boss_monster_id,
        BOSS_TARGET_SWITCH_INITIAL_DELAY_MS
    );
}
...
pub fn cleanup_ender_bolt_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    let bolt_schedulers_to_delete: Vec<u64> = ctx
        .db
    ...
    let active_bolt_attacks_to_delete: Vec<u64> = ctx
        .db
        .active_monster_attacks()
        .iter()
    ...
    if bolt_count > 0 || active_bolt_count > 0 {
        log::info!(
            "Cleaned up {} EnderBolt schedulers and {} active EnderBolt attacks for boss {}",
            bolt_count,
            active_bolt_count,
            boss_monster_id
        );
...
pub fn cleanup_ender_scythe_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    let spawn_schedulers_to_delete: Vec<u64> = ctx
        .db
        .ender_scythe_spawn_scheduler()
        .boss_monster_id()
        .filter(&boss_monster_id)
    ...
    let scythe_schedulers_to_delete: Vec<u64> = ctx
        .db
        .ender_scythe_scheduler()
        .boss_monster_id()
        .filter(&boss_monster_id)
    ...
    let active_scythe_attacks_to_delete: Vec<u64> = ctx
        .db
        .active_monster_attacks()
        .iter()
        .filter(|attack| {
...
pub fn cleanup_chaos_ball_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    let schedulers_to_delete: Vec<u64> = ctx
        .db
    ...
    if count > 0 || active_count > 0 {
        log::info!(
            "Cleaned up {} ChaosBall schedulers and {} active ChaosBall attacks for boss {}",
            count,
            active_count,
            boss_monster_id
        );
...
pub fn cleanup_void_zone_schedules(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    if count > 0 || active_count > 0 {
        log::info!(
            "Cleaned up {} VoidZone schedulers and {} active VoidZone attacks for boss {}",
            count,
            active_count,
            boss_monster_id
        );
...
pub fn cleanup_boss_target_switching(ctx: &ReducerContext, boss_monster_id: u32) {
    ...
    if count > 0 {
        log::info!(
            "Cleaned up {} boss target switching schedulers for boss {}",
            count,
            boss_monster_id
        );
    }
...
pub fn execute_boss_ender_chase_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    ...
    schedule_state_change(
        ctx,
        monster.monster_id,
        crate::monster_ai_defs::AIState::BossEnderIdle,
        BOSS_ENDER_CHASE_DURATION_MS,
    );
}
...
pub fn execute_boss_ender_vanish_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    ...
    schedule_state_change(
        ctx,
        monster.monster_id,
        crate::monster_ai_defs::AIState::BossEnderLurk,
        BOSS_ENDER_VANISH_DURATION_MS,
    );
}
...
pub fn execute_boss_ender_lurk_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    ...
    schedule_state_change(
        ctx,
        monster.monster_id,
        crate::monster_ai_defs::AIState::BossEnderTeleport,
        BOSS_ENDER_LURK_DURATION_MS,
    );
}
...
pub fn execute_boss_ender_teleport_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    ...
    schedule_state_change(
        ...
        monster.monster_id,
        crate::monster_ai_defs::AIState::BossEnderIdle,
        BOSS_ENDER_TELEPORT_DURATION_MS,
    );
}
...
pub fn execute_boss_ender_transform_behavior(ctx: &ReducerContext, monster: &crate::Monsters) {
    ...
    schedule_state_change(
        ...
        monster.monster_id,
        crate::monster_ai_defs::AIState::BossEnderIdle,
        BOSS_ENDER_TRANSFORM_DURATION_MS,
    );
}
...
pub fn schedule_random_boss_ender_pattern(ctx: &ReducerContext, monster_id: u32) {
    ...
    let available_patterns: Vec<crate::monster_ai_defs::AIState> = all_patterns
        .into_iter()
    ...
    if let Some(mut last_pattern_record) = last_pattern_opt {
        ...
        ctx.db
            .boss_ender_last_patterns()
            .monster_id()
            .update(last_pattern_record);
    } else {
        // Create new record for this boss
        ctx.db
            .boss_ender_last_patterns()
            .insert(BossEnderLastPattern {
                monster_id,
...
pub fn initialize_phase2_boss_ender_ai(ctx: &ReducerContext, monster_id: u32) {
    log::info!(
        "Initializing Phase 2 boss ender AI for monster {} (BossEnderIdle only, no patterns)",
        monster_id
    );
    ...
    if let Some(mut monster) = monster_opt {
        ...
        log::info!(
            "Phase 2 boss {} set to BossEnderIdle state (no automatic patterns)",
            monster_id
        );
    }
}
...
fn increase_monster_speed(ctx: &ReducerContext, monster: &crate::Monsters, multiplier: f32) {
    ...
    log::info!(
        "Monster {} speed increased to {} ({}x multiplier)",
        monster.monster_id,
        new_speed,
        multiplier
    );
...
fn change_monster_target_to_random_player(ctx: &ReducerContext, monster: &crate::Monsters) {
    ...
    if player_count == 0 {
        log::info!(
            "No players available to target for monster {}",
            monster.monster_id
        );
        return;
    }
    ...
    log::info!(
        "Monster {} target changed to player {} ({})",
        monster.monster_id,
        target_player.player_id,
        target_player.name
    );
...
fn teleport_monster_to_target(ctx: &ReducerContext, monster: &crate::Monsters) {
    ...
    let target_player = match target_player_opt {
        ...
        None => {
            log::info!(
                "Target player {} not found for monster {}, skipping teleport",
                monster.target_player_id,
                monster.monster_id
            );
            return;
        }
...
fn teleport_monster_to_position(ctx: &ReducerContext, monster_id: u32, position: &DbVector2) {
    ...
    if let Some(mut boid) = boid_opt {
        ...
        log::info!(
            "Monster {} teleported to position ({}, {})",
            monster_id,
            position.x,
            position.y
        );
...
fn schedule_state_change(
    ...
    log::info!(
        "Scheduled state change for monster {} to {:?} in {}ms",
        monster_id,
        target_state,
        delay_ms
    );
...
pub fn handle_ender_boss_chase_movement(
    ...
    if let Some(monster) = real_monster {
        let monster_type_name =
            crate::monster_types_def::get_monster_type_name(&monster.bestiary_id);
        if monster_type_name == "BossEnderPhase1" || monster_type_name == "BossEnderPhase2" {
            ...
            crate::monster_ai_defs::check_boss_chase_distance(
                ctx,
                monster_id,
                &monster_position,
                &target_position,
            ...
            if let Some(updated_monster) = updated_monster {
                let updated_behavior = crate::monster_ai_defs::get_movement_behavior_for_state(
                    &updated_monster.ai_state,
                );
