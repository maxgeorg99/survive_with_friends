...
use crate::{
    entity, get_world_cell_from_position, monster_damage, monsters_boid,
    spatial_hash_collision_checker, AttackType, DbVector2, Entity, DELTA_TIME,
    WORLD_CELL_BIT_SHIFT, WORLD_CELL_MASK, WORLD_GRID_HEIGHT, WORLD_GRID_WIDTH,
};
use spacetimedb::{
...
#[table(accessor = attack_data, public)]
...
#[table(accessor = attack_burst_cooldowns, scheduled(handle_attack_burst_cooldown), public)]
...
#[table(accessor = active_attacks, public)]
...
#[table(accessor = active_attack_cleanup, scheduled(cleanup_active_attack), public)]
...
#[table(accessor = player_scheduled_attacks, scheduled(server_trigger_attack), public)]
...
pub fn find_attack_data_by_type(
    ctx: &ReducerContext,
...
fn trigger_attack_projectile(
    ...
    let scheduled_attack = match scheduled_attack {
        ...
        None => {
            log::error!(
                "Scheduled attack not found for player {}, attack type {:?}",
                player_id,
                attack_type
            );
            return;
    ...
    if attack_type == AttackType::AngelStaff {
        ...
        ctx.db.active_attack_cleanup().insert(ActiveAttackCleanup {
            ...
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(scheduled_attack.duration as u64),
            ),
        });
        ...
        if attacker_pvp_enabled {
            for target_player in ctx.db.player().iter() {
                ...
                if distance_squared <= damage_radius_squared {
                    crate::core_game::damage_player(
                        ctx,
                        target_player.player_id,
                        scheduled_attack.damage as f32,
                    );
                }
            }
        }
        ...
        for monster in ctx.db.monsters().iter() {
            ...
            if distance_squared <= damage_radius_squared {
                let boss_defeated = crate::core_game::damage_monster(
                    ctx,
                    monster.monster_id,
    ...
    let direction = crate::attack_utils::determine_attack_direction(
        ctx,
        player_id,
        &attack_type,
        id_within_burst,
    ...
    let (projectile_position, final_direction) = if attack_type == AttackType::ThunderHorn {
        ...
        if let Some(target_position) = crate::attack_utils::find_random_target_in_radius(
            ctx,
            DbVector2::new(player_x, player_y),
            player_id,
            crate::attack_utils::THUNDER_HORN_TARGET_RADIUS,
        ) {
...
pub fn handle_attack_burst_cooldown(ctx: &ReducerContext, burst_cooldown: AttackBurstCooldown) {
    ...
    for attack in ctx
        ...
        .player_scheduled_attacks()
        .player_id()
        .filter(&burst_cooldown.player_id)
    {
    ...
    let scheduled_attack = match scheduled_attack {
        ...
        None => {
            log::error!(
                "Scheduled attack not found for player {}, attack type {:?}",
                burst_cooldown.player_id,
                burst_cooldown.attack_type.clone()
            );
            return;
        }
    };
    ...
    if attack_data.is_none() {
        log::error!(
            "Attack data not found for type {:?}",
            burst_cooldown.attack_type.clone()
        );
        return;
    }
    ...
    if burst_cooldown.remaining_shots == 0 {
        log::error!(
            "Remaining shots is 0 for player {}, attack type {:?}",
            burst_cooldown.player_id,
            burst_cooldown.attack_type.clone()
        );
        return;
    ...
    trigger_attack_projectile(
        ctx,
        burst_cooldown.player_id,
        burst_cooldown.attack_type.clone(),
        current_projectile_index,
        burst_cooldown.parameter_u,
        burst_cooldown.parameter_i,
    );
    ...
    if remaining_shots > 0 {
        ...
        ctx.db.attack_burst_cooldowns().insert(AttackBurstCooldown {
            ...
            scheduled_at: ScheduleAt::Time(
                ctx.timestamp + Duration::from_millis(scheduled_attack.fire_delay as u64),
            ),
        });
    }
}
...
pub fn server_trigger_attack(ctx: &ReducerContext, mut attack: PlayerScheduledAttack) {
    ...
    if attack_data.is_none() {
        log::error!(
            "Attack type {:?} not found when triggering attack",
    ...
    if attack.projectiles > 1 {
        if attack.fire_delay == 0 {
            ...
            ctx.db.attack_burst_cooldowns().insert(AttackBurstCooldown {
                ...
                scheduled_at: ScheduleAt::Time(
                    ctx.timestamp + Duration::from_millis(attack.fire_delay as u64),
                ),
            });
        }
    } else {
...
pub fn schedule_new_player_attack(
    ctx: &ReducerContext,
    player_id: u32,
    attack_type: AttackType,
    skill_level: u32,
    ...
    let attack_data = match attack_data {
        ...
        None => {
            log::error!(
                "Attack type {:?} not found when scheduling initial attacks",
                attack_type.clone()
            );
            return;
        }
    ...
    ctx.db
        ...
        .insert(PlayerScheduledAttack {
            ...
            scheduled_at: ScheduleAt::Interval(
                Duration::from_millis(attack_data.cooldown as u64).into(),
            ),
        });

    log::info!(
        "Scheduled initial attack of type {:?} for player {} with damage {}",
        attack_type.clone(),
        player_id,
        attack_data.damage
    );
...
pub fn cleanup_active_attack(ctx: &ReducerContext, cleanup: ActiveAttackCleanup) {
    ...
    let active_attack_opt = ctx
        .db
        .active_attacks()
        .active_attack_id()
        .find(&cleanup.active_attack_id);
    ...
    ctx.db
        .active_attacks()
        .active_attack_id()
        .delete(&cleanup.active_attack_id);
}
...
pub fn process_attack_movements(ctx: &ReducerContext) {
    ...
    for active_attack in ctx.db.active_attacks().iter() {
        ...
        let entity_opt = ctx
            ...
            .entity()
            .entity_id()
            .find(&updated_active_attack.entity_id);
        let entity = match entity_opt {
            Some(entity) => entity,
        ...
        if updated_active_attack.attack_type == AttackType::Shield {
            let player_cache_idx = match cache
                ...
                .get(&updated_active_attack.player_id)
            {
            ...
            let base_angle = parameter_angle
                + (2.0 * PI * updated_active_attack.id_within_burst as f64 / shield_count as f64);
            let shield_angle =
                base_angle + rotation_speed * updated_active_attack.ticks_elapsed as f64;
            ...
            updated_entity.position = DbVector2::new(player_x + offset_x, player_y + offset_y);
            ...
            for mid in 0..cache.monster.cached_count_monsters as usize {
                ...
                if distance_squared <= radius_sum * radius_sum {
                    ...
                    if distance > 0.0 {
                        ...
                        if let Some(mut boid) =
                            ctx.db.monsters_boid().monster_id().find(&monster_id)
                        {
                            let knockback_pos =
                                boid.position + (knockback_direction * knockback_strength);
            ...
            updated_entity.position.x = updated_entity
                .position
                .x
                .clamp(updated_entity.radius, world_size - updated_entity.radius);
            updated_entity.position.y = updated_entity
                .position
                .y
                .clamp(updated_entity.radius, world_size - updated_entity.radius);
            ...
            if updated_active_attack.parameter_u >= MAX_BOUNCES {
                ...
                let hit_boundary = updated_entity.position.x <= updated_entity.radius
                ...
                if hit_boundary {
                    ...
                    ctx.db
                        .entity()
                        .entity_id()
                        .delete(&updated_entity.entity_id);
                    ctx.db
                        .active_attacks()
                        .active_attack_id()
                        .delete(&updated_active_attack.active_attack_id);
            ...
            if updated_active_attack.attack_type == AttackType::Football {
                ...
                for mid in 0..cache.monster.cached_count_monsters as usize {
                    ...
                    if distance_squared <= radius_sum * radius_sum {
                        ...
                        {
                            let knockback_pos =
                                boid.position + (updated_entity.direction * knockback_strength);
                            boid.position = knockback_pos;
                            ctx.db.monsters_boid().monster_id().update(boid);
                        }
                    }
        ...
        let attacker_pvp_enabled = if let Some(&cache_idx) = cache
            .player
            .player_id_to_cache_index
            .get(&updated_active_attack.player_id)
        {
        ...
        let grid_cell_key =
            get_world_cell_from_position(updated_entity.position.x, updated_entity.position.y);
                ...
                while aid != -1 {
                    ...
                    if spatial_hash_collision_checker(px, py, pr, ax, ay, ar) {
                        ...
                        let active_attack_opt = ctx
                            ...
                            .active_attacks()
                            .active_attack_id()
                            .find(&cache.attack.keys_attack[aid_usize]);
                        let active_attack = match active_attack_opt {
                            Some(attack) => attack,
                            None => {
