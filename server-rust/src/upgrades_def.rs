use crate::{
    account, active_attack_cleanup, active_attacks, attack_burst_cooldowns, attack_data,
    boss_spawn_timer, entity, game_state, gems, monster_damage, monster_hit_cleanup,
    monster_spawn_timer, monster_spawners, monsters, monsters_boid, player,
    player_scheduled_attacks, AttackType, Player, PlayerClass, WORLD_CELL_SIZE,
};
use spacetimedb::{
...
#[table(accessor = upgrade_options, public)]
...
#[table(accessor = chosen_upgrades, public)]
...
pub fn draw_upgrade_options(ctx: &ReducerContext, player_id: u32) {
    ...
    let existing_options: Vec<_> = ctx
        .db
        .upgrade_options()
        .player_id()
        .filter(&player_id)
    ...
    log::info!(
        "Selected upgrade types for player {}: {:?}",
        player_id,
        selected_types
    );
...
fn create_upgrade_option_data(
    ...
    if is_attack_upgrade {
        ...
        if !player_has_attack {
            log::info!(
                "Player {} doesn't have {:?} yet, offering as new attack",
                player_id,
                attack_type
            );
    ...
    match upgrade_type {
        UpgradeType::MaxHp => UpgradeOptionData {
            upgrade_id: 0,
            player_id,
            upgrade_index: 0,
            upgrade_type,
        ...
        UpgradeType::HpRegen => UpgradeOptionData {
            upgrade_id: 0,
            player_id,
            upgrade_index: 0,
            upgrade_type,
        ...
        UpgradeType::Speed => UpgradeOptionData {
            upgrade_id: 0,
            player_id,
            upgrade_index: 0,
            upgrade_type,
        ...
        UpgradeType::Armor => UpgradeOptionData {
            upgrade_id: 0,
            player_id,
            upgrade_index: 0,
            upgrade_type,
...
fn generate_attack_upgrade(
    ctx: &ReducerContext,
    player_id: u32,
    attack_type: u32,
    possible_stats: Vec<(AttackStat, u32)>,
    upgrade_type: UpgradeType,
) -> UpgradeOptionData {
    ...
    let filtered_stats =
        filter_stats_by_radius_limit(ctx, player_id, attack_type_enum.clone(), possible_stats);

    // If no stats remain after filtering, fallback to damage upgrade
    let final_stats = if filtered_stats.is_empty() {
        log::warn!(
            "No valid stats remain for attack {:?} after filtering, falling back to damage upgrade",
            attack_type_enum
        );
    ...
    log::info!(
        "Generated attack upgrade: {:?}, Stat: {:?}, Value: {}",
        upgrade_type,
        chosen_stat,
        upgrade_value
    );
...
pub fn choose_upgrade(ctx: &ReducerContext, player_id: u32, upgrade_index: u32) {
    ...
    if identity_account.current_player_id != player_id {
        ...
        if let Some(requested_player) = ctx.db.player().player_id().find(&player_id) {
            ...
            log::error!(
                "Requested Player ID {} does not exist in the database",
                player_id
            );
        }
        ...
        if identity_account.current_player_id > 0 {
            if let Some(caller_player) = ctx
                .db
                .player()
                .player_id()
                .find(&identity_account.current_player_id)
            {
                log::error!(
    ...
    let upgrade_options_data: Vec<_> = ctx
        .db
        .upgrade_options()
        .player_id()
        .filter(&player_id)
    ...
    log::info!(
        "Player {} has {} upgrade options available:",
        player_id,
        upgrade_options_data.len()
    );
    ...
    if upgrade_options_data.is_empty() {
        log::error!(
            "Player {} has no available upgrades to choose from",
            player_id
        );
    ...
    log::info!(
        "Deleted {} upgrade options for player {}",
        delete_count,
        player_id
    );
    ...
    if !upgrade.is_attack_upgrade {
        ...
        match upgrade.upgrade_type {
            ...
            UpgradeType::HpRegen => {
                ...
                log::info!(
                    "Increased player {} HP regen by {} to {}",
                    player_id,
                    upgrade.value,
                    player.hp_regen
                );
            ...
            _ => {
                log::info!(
                    "Unknown non-attack upgrade type: {:?}",
                    upgrade.upgrade_type
                );
            }
        }
        ...
        if attack_type == AttackType::Shield && (upgrade.projectiles > 0 || upgrade.is_new_attack) {
            let new_shield_count = if upgrade.is_new_attack {
                2
            } else {
                upgrade.projectiles
            };
        ...
        if existing_attack.is_none() {
            ...
            log::info!(
                "Player {} doesn't have attack {:?} yet, scheduling new attack",
                player_id,
                attack_type
            );
            schedule_new_player_attack(ctx, player_id, attack_type, 1);
        ...
        if upgrade.damage > 0 {
            ...
            log::info!(
                "Increased attack {:?} damage by {} to {}",
                attack_type,
                upgrade.damage,
                modified_attack.damage
            );
        }
        ...
        if upgrade.cooldown_ratio > 0 {
            ...
            log::info!(
                "Increased attack {:?} fire rate by {}%",
                attack_type,
                upgrade.cooldown_ratio
            );
        }
        ...
        if upgrade.projectiles > 0 {
            ...
            update_scheduled_attack = true;
            log::info!(
                "Increased attack {:?} projectiles by {} to {}",
                attack_type,
                upgrade.projectiles,
                modified_attack.projectiles
            );
        }
        ...
        if upgrade.speed > 0 {
            ...
            update_scheduled_attack = true;
            log::info!(
                "Increased attack {:?} speed by {}% to {}",
                attack_type,
                upgrade.speed,
                modified_attack.speed
            );
        }
        ...
        if upgrade.radius > 0 {
            ...
            if attack_type != AttackType::AngelStaff && new_radius > MAX_RADIUS_LIMIT {
                ...
                if attack_type == AttackType::AngelStaff {
                    log::info!(
                        "Increased Angel Staff radius by {} to {} (no limit)",
                        upgrade.radius,
                        modified_attack.radius
                    );
                } else {
                    log::info!(
                        "Increased attack {:?} radius by {} to {}",
                        attack_type,
                        upgrade.radius,
                        modified_attack.radius
                    );
        ...
        if update_schedule_interval && cooldown_reduction > 0 {
            ...
            let new_scheduled_attack = crate::PlayerScheduledAttack {
                ...
                scheduled_at: ScheduleAt::Interval(
                    Duration::from_millis(new_cooldown as u64).into(),
                ),
            };

            ctx.db
                .player_scheduled_attacks()
                .insert(new_scheduled_attack);

            log::info!(
                "Updated attack cooldown: reduced from {}ms to {}ms (stacking upgrade)",
                current_cooldown,
                new_cooldown
            );
        }
        ...
        if update_scheduled_attack {
            ctx.db
                .player_scheduled_attacks()
                .scheduled_id()
                .update(modified_attack);
            log::info!(
                "Updated scheduled attack for player {}, attack type {:?}",
                player_id,
                attack_type
            );
        }
...
fn get_attack_type_from_upgrade(upgrade_type: &UpgradeType) -> AttackType {
    match upgrade_type {
        ...
        _ => panic!(
            "Cannot convert upgrade type {:?} to attack type",
            upgrade_type
        ),
    }
}
...
pub fn reroll_upgrades(ctx: &ReducerContext, player_id: u32) {
    ...
    log::info!(
        "Deleted {} upgrade options for player {} during reroll",
        delete_count,
        player_id
    );
...
fn find_attack_data_by_type(
    ctx: &ReducerContext,
    attack_type: &AttackType,
) -> Option<crate::AttackData> {
...
pub fn cleanup_player_upgrade_options(ctx: &ReducerContext, player_id: u32) {
    ...
    let upgrade_options_to_delete: Vec<_> = ctx
        .db
        .upgrade_options()
        .player_id()
        .filter(&player_id)
...
fn filter_stats_by_radius_limit(
    ctx: &ReducerContext,
    player_id: u32,
    attack_type: AttackType,
    mut possible_stats: Vec<(AttackStat, u32)>,
) -> Vec<(AttackStat, u32)> {
    // Angel Staff is exempt from radius limits
    if attack_type != AttackType::AngelStaff
        && is_attack_radius_at_limit(ctx, player_id, attack_type.clone())
    {
        ...
        log::info!(
            "Player {} attack {:?} radius at limit, filtered out radius upgrades",
            player_id,
            attack_type
        );
    }
