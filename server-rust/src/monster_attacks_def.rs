use crate::{
    get_world_cell_from_position, monsters, monsters_boid, player, spatial_hash_collision_checker,
    DbVector2, MonsterAttackType, DELTA_TIME, WORLD_CELL_BIT_SHIFT, WORLD_CELL_MASK,
    WORLD_GRID_HEIGHT, WORLD_GRID_WIDTH,
};
use spacetimedb::{
...
#[table(accessor = imp_attack_scheduler, scheduled(trigger_imp_attack), public)]
...
#[table(accessor = active_monster_attacks, scheduled(expire_monster_attack), public)]
...
pub fn process_monster_attack_movements(ctx: &ReducerContext) {
    ...
    for active_monster_attack in ctx.db.active_monster_attacks().iter() {
        ...
        crate::boss_ender_defs::handle_boss_attack_movement(
            ctx,
            &mut updated_active_monster_attack,
        );
        ...
        match updated_active_monster_attack.monster_attack_type {
            ...
            _ => {
                ...
                updated_active_monster_attack.position =
                    updated_active_monster_attack.position + move_offset;
            }
        }

        // Update collision cache
        let cache_idx = cache.monster_attack.cached_count_monster_attacks as usize;
        cache.monster_attack.keys_monster_attack[cache_idx] =
            updated_active_monster_attack.active_monster_attack_id as u32;
        cache.monster_attack.pos_x_monster_attack[cache_idx] =
            updated_active_monster_attack.position.x;
        cache.monster_attack.pos_y_monster_attack[cache_idx] =
            updated_active_monster_attack.position.y;
        cache.monster_attack.radius_monster_attack[cache_idx] =
            updated_active_monster_attack.radius;

        let grid_cell_key = get_world_cell_from_position(
            updated_active_monster_attack.position.x,
            updated_active_monster_attack.position.y,
        );
        cache.monster_attack.nexts_monster_attack[cache_idx] =
            cache.monster_attack.heads_monster_attack[grid_cell_key as usize];
        ...
        ctx.db
            ...
            .update(updated_active_monster_attack);
    }
}
            ...
            for dx in -1..=1 {
                ...
                while aid != -1 {
                    ...
                    if spatial_hash_collision_checker(px, py, pr, ax, ay, ar) {
                        ...
                        let active_monster_attack_opt = ctx
                            .db
                            .active_monster_attacks()
                            .active_monster_attack_id()
                            .find(&attack_id);
                        ...
                        if active_monster_attack.piercing {
                            ...
                            ctx.db
                                .active_monster_attacks()
                                .active_monster_attack_id()
                                .delete(&active_monster_attack.active_monster_attack_id);
                        }
...
pub fn expire_monster_attack(ctx: &ReducerContext, attack: ActiveMonsterAttack) {
    ...
    log::info!(
        "Monster attack {} ({:?}) expired at position ({}, {})",
...
pub fn spawn_imp_bolt(
    ctx: &ReducerContext,
    spawn_position: DbVector2,
    target_player_id: u32,
    attacking_monster_id: u32,
) {
    ...
    let target_player = match target_player_opt {
        ...
        None => {
            log::error!(
                "Cannot spawn ImpBolt: target player {} not found",
                target_player_id
            );
            return;
        }
    ...
    let (base_damage, is_shiny) = match attacking_monster {
        ...
        None => {
            log::warn!(
                "Attacking monster {} not found, using default damage",
    ...
    if is_shiny {
        ...
        log::info!(
            "Spawned ImpBolt {} at ({}, {}) targeting player {} (expires in {}ms)",
            active_monster_attack.active_monster_attack_id,
            spawn_position.x,
            spawn_position.y,
            target_player_id,
...
pub fn trigger_imp_attack(ctx: &ReducerContext, scheduler: ImpAttackScheduler) {
    ...
    let imp_opt = ctx
        ...
        .monsters()
        .monster_id()
        .find(&scheduler.imp_monster_id);
    let imp = match imp_opt {
        Some(monster) => monster,
        None => {
            // Imp is dead - don't reschedule, just log cleanup
            log::info!(
                "Imp {} no longer exists, removing attack scheduler",
                scheduler.imp_monster_id
            );
            return;
        }
    };
    ...
    let imp_position = match boid_opt {
        ...
        None => {
            ...
            log::info!(
                "Imp {} has no boid, removing attack scheduler",
                scheduler.imp_monster_id
            );
            return;
        }
        ...
        None => {
            ...
            match new_target_player {
                ...
                None => {
                    ...
                    log::info!(
                        "No target players found for Imp {}, skipping attack",
                        scheduler.imp_monster_id
                    );
    ...
    log::info!(
        "Imp {} fired ImpBolt at player {} from position ({}, {})",
        scheduler.imp_monster_id,
        target_player.player_id,
        imp_position.x,
        imp_position.y
...
pub fn start_imp_attack_schedule(ctx: &ReducerContext, imp_monster_id: u32) {
    ...
    log::info!(
        "Started attack schedule for Imp {} (first attack in {}ms)",
        imp_monster_id,
        initial_delay_ms
    );
}
...
pub fn cleanup_imp_attack_schedule(ctx: &ReducerContext, imp_monster_id: u32) {
    ...
    if count > 0 {
        log::info!(
            "Cleaned up {} attack schedulers for dead Imp {}",
            count,
            imp_monster_id
        );
    }
