...
class AuthManager {
    ...
    constructor() {
        ...
        console.log(`[OIDC] Running in ${this.isDevelopment ? 'DEVELOPMENT' : 'PRODUCTION'} mode`);
        ...
        if (!this.isDevelopment) {
            ...
            this.userManager.events.addUserLoaded((user) => {
                console.log("[OIDC] User loaded:", user?.profile?.sub);
                this.user = user;
            });
            ...
            console.log("[OIDC] Development mode: Creating mock user");
    ...
    async initialize() {
        if (this.isDevelopment) {
            console.log("[OIDC] Development mode: Skipping real authentication");
            console.log("[OIDC] Mock user ready:", this.user?.profile?.sub);
            return;
        }
        ...
        try {
            ...
            if (window.location.pathname === '/callback') {
                ...
                console.log("[OIDC] Callback processed, redirecting to main page");
                return;
            }
            ...
            if (!this.user || this.user.expired) {
                console.log("[OIDC] No valid authenticated user found, starting authentication...");
                console.log("[OIDC] Redirecting to authentication provider...");
                await this.userManager.signinRedirect();
            } else {
                console.log("[OIDC] User already authenticated:", this.user.profile?.sub);
                console.log("[OIDC] Access token expires at:", new Date(this.user.expires_at * 1000));
            }
        } catch (error) {
            console.error("[OIDC] Authentication error:", error);
            console.log("[OIDC] Continuing without authentication for debugging...");
        }
    }
...
// Initialize auth before starting the game
