...
const MONSTER_ASSET_KEYS: Record<string, string> = {
    "Rat": 'monster_rat',
    "Slime": 'monster_slime',
    "Orc": 'monster_orc'
};
...
export default class GameScene extends Phaser.Scene {
    ...
    handleEntityUpdate(entityData: Entity) {
        ...
        // Check if this entity belongs to a monster
    ...
    createMonsterSprite(monsterData: Monsters, position: { x: number, y: number }) {
        ...
        if (this.monsters.has(monsterData.monsterId)) {
            ...
            // Add shadow
            const shadow = this.add.image(0, MONSTER_SHADOW_OFFSET_Y, SHADOW_ASSET_KEY)
                .setAlpha(SHADOW_ALPHA)
                .setDepth(0);
            ...
            // Default maximum health values based on monster type
            let maxHP = 100;
            switch(monsterType) {
                case 'Rat': maxHP = 10; break;
                case 'Slime': maxHP = 25; break;
                case 'Orc': maxHP = 50; break;
                default: maxHP = 100;
            }
